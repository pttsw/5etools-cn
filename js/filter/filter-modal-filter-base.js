...
export class ModalFilterBase {
	...
	async pPopulateWrapper (wrp, opts = null) {
		...
		const ovlLoading = ee`<div class="ve-w-100 ve-h-100 ve-flex-vh-center"><i class="ve-dnd-font ve-muted">加载中...</i></div>`.appendTo(wrp);
		...
		const btnReset = opts.btnReset || ee`<button class="ve-btn ve-btn-default">重置</button>`;
		...
		const btnConfirm = opts.isBuildUi ? null : ee`<button class="ve-btn ve-btn-default">${I18nUtil.get("common.button.confirm")}</button>`;
