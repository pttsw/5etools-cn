...
export class FilterBox extends ProxyBase {
	...
	render () {
		...
		if (this._wrpFormTop || this._btnOpen) {
			if (!this._btnOpen) {
				this._btnOpen = ee`<button class="ve-btn ve-btn-default ${this._isCompact ? "px-2" : ""}">筛选</button>`
					.prependTo(this._wrpFormTop);
	...
	async _render_pRenderModal () {
		...
		const btnReset = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-3" title="${TITLE_BTN_RESET}">重置</button>`
			.onn("click", evt => this.reset(evt.shiftKey));
		...
		const btnSave = ee`<button class="ve-btn ve-btn-primary fltr__btn-close mr-2">保存</button>`
			.onn("click", () => this._modalMeta.doClose(true));

		const btnCancel = ee`<button class="ve-btn ve-btn-default fltr__btn-close">取消</button>`
		...
		ee(this._modalMeta.eleModal)`<div class="split mb-2 mt-2 ve-flex-v-center mobile-sm__ve-flex-col">
			<div class="ve-flex-v-baseline mobile-sm__ve-flex-col">
				<h4 class="m-0 mr-2 mobile-sm__mb-2">筛选</h4>
