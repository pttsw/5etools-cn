...
export class FilterBox extends ProxyBase {
	...
	render () {
		...
		if (this._wrpFormTop || this._btnOpen) {
			if (!this._btnOpen) {
				this._btnOpen = veT`<button class="ve-btn ve-btn-default ${this._isCompact ? "ve-px-2" : ""}">筛选</button>`
					.vee.prependTo(this._wrpFormTop);
	...
	async _render_pRenderModal () {
		...
		this._metaIptSearch = ComponentUiUtil.getIptStr(
			...
			{decorationRight: "clear", asMeta: true, html: `<input class="ve-form-control ve-input-xs" placeholder="搜索...">`},
		);
		...
		const btnShowAllFilters = veT`<button class="ve-btn ve-btn-xs ve-btn-default">显示全部</button>`
			.vee.onn("click", () => this.showAllFilters());
		const btnHideAllFilters = veT`<button class="ve-btn ve-btn-xs ve-btn-default">隐藏全部</button>`
			.vee.onn("click", () => this.hideAllFilters());

		const btnReset = veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-3" title="${TITLE_BTN_RESET}">重置</button>`
			.vee.onn("click", evt => this.reset(evt.shiftKey));

		const btnSnapshotManager = this._snapshotManager.getBtn();

		const btnSettings = veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="设置"><span class="glyphicon glyphicon-cog"></span></button>`
			.vee.onn("click", () => this._pOpenSettingsModal());

		const btnSaveAlt = veT`<button class="ve-btn ve-btn-xs ve-btn-primary" title="保存"><span class="glyphicon glyphicon-ok"></span></button>`
		...
		const btnSave = veT`<button class="ve-btn ve-btn-primary ve-fltr__btn-close ve-mr-2">保存</button>`
			.vee.onn("click", () => this._modalMeta.doClose(true));

		const btnCancel = veT`<button class="ve-btn ve-btn-default ve-fltr__btn-close">取消</button>`
		...
		veT(this._modalMeta.eleModal)`<div class="ve-split ve-mb-2 ve-mt-2 ve-flex-v-center ve-mobile-sm__flex-col">
			<div class="ve-flex-v-baseline ve-mobile-sm__flex-col">
				<h4 class="ve-m-0 ve-mr-2 ve-mobile-sm__mb-2">筛选</h4>
	...
	async _pOpenSettingsModal () {
		...
		UiUtil.getAddModalRowCb(eleModalInner, "默认反选预发布资源", this._meta, "isPrereleaseDefaultHidden");
		UiUtil.getAddModalRowCb(eleModalInner, "默认反选自制资源", this._meta, "isBrewDefaultHidden");

		UiUtil.addModalSep(eleModalInner);

		UiUtil.getAddModalRowHeader(eleModalInner, "概览隐藏筛选器...", {helpText: "The summary is the small red and blue button panel which appear below the search bar."});
		...
		rowResetAlwaysSave.vee.appends(`<span>总是在关闭时保存</span>`);
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">重置</button>`
	...
	async _pOpenCombineAsModal () {
		...
		const btnReset = veT`<button class="ve-btn ve-btn-xs ve-btn-default">重置</button>`
			.vee.onn("click", () => {
