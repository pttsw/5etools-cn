...
export class FilterBox extends ProxyBase {
	...
	render () {
		...
		if (this._wrpFormTop || this._btnOpen) {
			if (!this._btnOpen) {
				this._btnOpen = ee`<button class="ve-btn ve-btn-default ${this._isCompact ? "ve-px-2" : ""}">筛选</button>`
					.prependTo(this._wrpFormTop);
	...
	async _render_pRenderModal () {
		...
		const btnShowAllFilters = ee`<button class="ve-btn ve-btn-xs ve-btn-default">显示全部</button>`
			.onn("click", () => this.showAllFilters());
		const btnHideAllFilters = ee`<button class="ve-btn ve-btn-xs ve-btn-default">隐藏全部</button>`
			.onn("click", () => this.hideAllFilters());

		const btnReset = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-3" title="${TITLE_BTN_RESET}">重置</button>`
			.onn("click", evt => this.reset(evt.shiftKey));

		const btnSnapshotManager = this._snapshotManager.getBtn();

		const btnSettings = ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="设置"><span class="glyphicon glyphicon-cog"></span></button>`
			.onn("click", () => this._pOpenSettingsModal());

		const btnSaveAlt = ee`<button class="ve-btn ve-btn-xs ve-btn-primary" title="保存"><span class="glyphicon glyphicon-ok"></span></button>`
		...
		const btnSave = ee`<button class="ve-btn ve-btn-primary ve-fltr__btn-close ve-mr-2">保存</button>`
			.onn("click", () => this._modalMeta.doClose(true));

		const btnCancel = ee`<button class="ve-btn ve-btn-default ve-fltr__btn-close">取消</button>`
	...
	async _pOpenSettingsModal () {
		...
		rowResetAlwaysSave.appends(`<span>总是在关闭时保存</span>`);
		ee`<button class="ve-btn ve-btn-xs ve-btn-default">重置</button>`
	...
	async _pOpenCombineAsModal () {
		...
		const btnReset = ee`<button class="ve-btn ve-btn-xs ve-btn-default">重置</button>`
			.onn("click", () => {
