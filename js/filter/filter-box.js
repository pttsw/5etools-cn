...
export class FilterBox extends ProxyBase {
	...
	async _render_pRenderModal () {
		...
		const $btnShowAllFilters = $(`<button class="ve-btn ve-btn-xs ve-btn-default">显示全部</button>`)
			.click(() => this.showAllFilters());
		const $btnHideAllFilters = $(`<button class="ve-btn ve-btn-xs ve-btn-default">隐藏全部</button>`)
			.click(() => this.hideAllFilters());

		const $btnReset = $(`<button class="ve-btn ve-btn-xs ve-btn-default mr-3" title="${TITLE_BTN_RESET}">重置</button>`)
			.click(evt => this.reset(evt.shiftKey));
		...
		const $btnSave = $(`<button class="ve-btn ve-btn-primary fltr__btn-close mr-2">保存</button>`)
			.click(() => this._modalMeta.doClose(true));

		const $btnCancel = $(`<button class="ve-btn ve-btn-default fltr__btn-close">取消</button>`)
	...
	async _pOpenSettingsModal () {
		...
		UiUtil.$getAddModalRowCb($modalInner, "默认反选预发布资源", this._meta, "isPrereleaseDefaultHidden");
		...
		$(`<button class="ve-btn ve-btn-xs ve-btn-default">重置</button>`)
	...
	async _pOpenCombineAsModal () {
		...
		const $btnReset = $(`<button class="ve-btn ve-btn-xs ve-btn-default">重置</button>`)
			.click(() => {
