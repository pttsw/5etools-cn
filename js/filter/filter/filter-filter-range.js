...
export class RangeFilter extends FilterBase {
	...
	_$getHeaderControls () {
		const $btnForceMobile = ComponentUiUtil.$getBtnBool(
			...
			"isUseDropdowns",
			{
				$ele: $(`<button class="ve-btn ve-btn-default ve-btn-xs mr-2">以下拉列表展示</button>`),
				stateName: "uiMeta",
				stateProp: "_uiMeta",
			},
		);
		const $btnReset = $(`<button class="ve-btn ve-btn-default ve-btn-xs">重置</button>`).click(() => this.reset());
