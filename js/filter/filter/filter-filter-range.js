...
export class RangeFilter extends FilterBase {
	...
	_getHeaderControls () {
		const btnForceMobile = ComponentUiUtil.getBtnBool(
			...
			"isUseDropdowns",
			{
				ele: veT`<button class="ve-btn ve-btn-default ve-btn-xs ve-mr-2">以下拉列表展示</button>`,
				stateName: "uiMeta",
				stateProp: "_uiMeta",
			},
		);
		const btnReset = veT`<button class="ve-btn ve-btn-default ve-btn-xs">重置</button>`.vee.onn("click", () => this.reset());
