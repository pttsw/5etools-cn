...
class TimeTrackerRoot extends TimeTrackerBase {
	...
	render (eleParent) {
		...
		const btnShowClock = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2" title="时钟"><span class="glyphicon glyphicon-time"></span></button>`
			.onn("click", () => this._state.tab = 0);
		const btnShowCalendar = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-3" title="日历"><span class="glyphicon glyphicon-calendar"></span></button>`
			.onn("click", () => this._state.tab = 1);
		const btnShowSettings = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-3" title="设置"><span class="glyphicon glyphicon-cog"></span></button>`
		...
		const btnReset = ee`<button class="ve-btn ve-btn-xs ve-btn-danger" title="重置时钟/日历时间为第一天"><span class="glyphicon glyphicon-refresh"></span></button>`
			.onn("click", async () => {
...
class TimeTrackerRoot_Clock extends TimeTrackerComponent {
	...
	render (eleParent, parent) {
		...
		const btnNextSunrise = ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="跳过到下一个日出前的时间。如果已经到凌晨了，则会跳到当天早上，否则调到转天早上。">下一个日出</button>`
			.onn("click", () => {
		...
		const btnNextDay = ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="跳过到下一个凌晨前的时间。">下一天</button>`
		...
		const btnSubDay = ee`<button class="ve-btn ve-btn-xxs ve-btn-default dm-time__btn-day"  title="减一天(按住SHIFT减五天)">-</button>`
		...
		const btnAddDay = ee`<button class="ve-btn ve-btn-xxs ve-btn-default dm-time__btn-day" title="加一天(按住SHIFT加五天)">+</button>`
		...
		const btnAddHour = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--top" title="加一小时(按住SHIFT加5小时,按住CTRL加12小时)">+</button>`
		...
		const btnSubHour = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--bottom" title="减一小时(按住SHIFT减5小时,按住CTRL减12小时)">-</button>`
		...
		const btnAddMinute = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--top" title="加一分钟(按住SHIFT加5分钟,按住CTRL加15分钟,同时按住加30分钟)">+</button>`
		...
		const btnSubMinute = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--bottom" title="减一分钟(按住SHIFT减5分钟,按住CTRL减15分钟,同时按住减30分钟)">-</button>`
		...
		const btnAddSecond = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--top" title="加一秒(按住SHIFT加5秒,按住CTRL加15秒,同时按住加30秒)">+</button>`
		...
		const btnSubSecond = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--bottom" title="减一秒(按住SHIFT减5秒,按住CTRL减15秒,同时按住减30秒)">-</button>`
		...
		const btnAddLongRest = ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="添加一次长休时间 (按住SHIFT改为减去)">长休</button>`
		...
		const btnAddShortRest = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2" title="添加一次短休时间 (按住SHIFT改为减去)">短休</button>`
		...
		const btnAddTurn = ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="添加一个回合时间 (按住SHIFT改为减去)">回合</button>`
...
class TimeTrackerRoot_Clock_Weather extends TimeTrackerComponent {
	render (eleParent, parent) {
		...
		const btnRandomise = ee`<button class="ve-btn ve-btn-xxs ve-btn-default dm-time__btn-random-weather" title="随机生成天气 (按住SHIFT重新使用之前的设置)"><span class="fal fa-dice"></span></button>`
			.onn("click", async evt => {
...
class TimeTrackerRoot_Clock_RandomWeather extends BaseComponent {
	...
	render (eleModalInner, doClose) {
		...
		const btnOk = ee`<button class="ve-btn ve-btn-default">确认并随机生成天气</button>`
			.onn("click", () => {
...
class TimeTrackerRoot_Calendar extends TimeTrackerComponent {
	...
	render (eleParent, parent) {
		...
		const btnBrowseMode = ComponentUiUtil.getBtnBool(
			...
			"isBrowseMode",
			{
				ele: ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="当启用此选项时，当前日历试图将被暂存，你可以随意浏览其他日期，当你浏览结束后，你可以禁用此选项以返回保存的视图。">浏览模式</button>`,
				fnHookPost: val => {
	...
	static getDateControls (parent, opts) {
		...
		const btnSubDay = opts.isHideDays ? null : ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一天(按住SHIFT减5天)">\u2212D</button>`
			.onn("click", evt => doModTime(-1 * getTimeInfo().secsPerDay * (evt.shiftKey ? 5 : 1)));
		const btnAddDay = opts.isHideDays ? null : ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一天(按住SHIFT加5天)">D+</button>`
			.onn("click", evt => doModTime(getTimeInfo().secsPerDay * (evt.shiftKey ? 5 : 1)));

		const btnSubWeek = opts.isHideWeeks ? null : ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一周(按住SHIFT减5周)">\u2212W</button>`
			.onn("click", evt => doModTime(-1 * getTimeInfo().secsPerWeek * (evt.shiftKey ? 5 : 1)));
		const btnAddWeek = opts.isHideWeeks ? null : ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一周(按住SHIFT加5周)">W+</button>`
		...
		const btnSubMonth = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一个月(按住SHIFT减五个月)">\u2212M</button>`
			.onn("click", evt => doModMonths(evt.shiftKey ? -5 : -1));
		const btnAddMonth = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一个月(按住SHIFT加五个月)">M+</button>`
			.onn("click", evt => doModMonths(evt.shiftKey ? 5 : 1));

		const btnSubYear = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一年(按住SHIFT减五年)">\u2212Y</button>`
			.onn("click", evt => doModTime(-1 * getTimeInfo().secsPerYear * (evt.shiftKey ? 5 : 1)));
		const btnAddYear = ee`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一年(按住SHIFT加五年)">Y+</button>`
...
class TimeTrackerRoot_Settings_Event extends TimeTrackerComponent {
	...
	doOpenEditModal (overlayColor = "transparent") {
		...
		const btnOk = ee`<button class="ve-btn ve-btn-default" data-i18n='common.button.save'></button>`
			.onn("click", () => doClose(true));
