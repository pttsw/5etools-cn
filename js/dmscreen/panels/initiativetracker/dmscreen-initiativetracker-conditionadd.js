...
export class InitiativeTrackerConditionAdd extends BaseComponent {
	...
	_render_getStgIpts ({rdState}) {
		...
		const iptTurns = ComponentUiUtil.getIptInt(this, "turns", null, {isAllowNull: true, fallbackOnNaN: null, html: `<input class="ve-form-control" placeholder="无限制">`})
			.vee.onn("keydown", evt => {
				if (evt.key !== "Enter") return;
		...
		const btnSave = veT`<button class="ve-btn ve-btn-default ve-w-100" title="保存为自定义状态"><span class="glyphicon glyphicon-floppy-disk"></span></button>`
			.vee.onn("click", () => {
		...
		return veT`
			<div class="ve-flex-v-center ve-mb-2">
				<div class="ve-small-caps ve-col-5 ve-pr-1">名称</div>
				<div class="ve-small-caps ve-col-2 ve-px-1">颜色</div>
				<div class="ve-small-caps ve-col-4 ve-px-1">持续时间</div>
	...
	_render_getStgSubmit ({rdState}) {
		const btnAdd = veT`<button class="ve-btn ve-btn-primary ve-w-100">设置状态</button>`
