...
class _RenderableCollectionRowDataActive extends RenderableCollectionRowDataBase {
	...
	_pPopulateRow_monster ({comp, wrpLhs, isMon, mon, fluff}) {
		...
		const btnRename = veT`<button class="ve-btn ve-btn-default ve-btn-xs dm-init-lockable dm-init__btn-creature" title="重命名(按住SHIFT来重置)" tabindex="-1"><span class="glyphicon glyphicon-pencil"></span></button>`
		...
		const btnDuplicate = veT`<button class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable dm-init__btn-creature" title="添加另一个(按住SHIFT来为新的掷骰)" tabindex="-1"><span class="glyphicon glyphicon-plus"></span></button>`
			.vee.onn("click", async (evt) => {
				if (this._comp._state.isLocked) return;
	...
	_pPopulateRow_conditions ({comp, wrpLhs}) {
		const btnAddCond = veT`<button class="ve-btn ve-btn-warning ve-btn-xs dm-init__row-btn dm-init__row-btn-flag" title="${I18nUtil.get("page.dmscreen.add_contition")}" tabindex="-1"><span class="glyphicon glyphicon-flag"></span></button>`
			.vee.onn("click", async () => {
	...
	_pPopulateRow_btns ({comp, entity, wrpRhs}) {
		const btnVisible = InitiativeTrackerUi.getBtnPlayerVisible({
			...
			.vee.tooltip(I18nUtil.get("page.dmscreen.shown_in_player_view"))
			.vee.appendTo(wrpRhs);

		veT`<button class="ve-btn ve-btn-danger ve-btn-xs dm-init__row-btn dm-init-lockable" title="${I18nUtil.get("common.button.delete")} (${I18nUtil.get("page.dmscreen.shift_to_delete_similar")})" tabindex="-1"><span class="glyphicon glyphicon-trash"></span></button>`
			.vee.appendTo(wrpRhs)
			.vee.onn("click", evt => {
				if (this._comp._state.isLocked) return;
