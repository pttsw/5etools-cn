...
class InitiativeTrackerComponent extends BaseComponent {
	...
	_render_getWrpButtonsSort () {
		const btnSortAlpha = veT`<button title="${I18nUtil.get("page.dmscreen.sort_alphabetically")}" class="ve-btn ve-btn-default ve-btn-xs"><span class="glyphicon glyphicon-sort-by-alphabet"></span></button>`
			.vee.onn("click", () => {
		...
		const btnSortNumber = veT`<button title="${I18nUtil.get("page.dmscreen.sort_numerically")}" class="ve-btn ve-btn-default ve-btn-xs"><span class="glyphicon glyphicon-sort-by-order"></span></button>`
			.vee.onn("click", () => {
	...
	_render_getWrpFooter ({wrpTracker, doUpdateExternalStates}) {
		const btnAdd = veT`<button class="ve-btn ve-btn-primary ve-btn-xs dm-init-lockable" title="${I18nUtil.get("page.dmscreen.add_player")}"><span class="glyphicon glyphicon-plus"></span></button>`
			.vee.onn("click", async () => {
		...
		const btnAddMonster = veT`<button class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable ve-mr-2" title="${I18nUtil.get("page.dmscreen.add_creature")}"><span class="glyphicon glyphicon-print"></span></button>`
			.vee.onn("click", async () => {
				if (this._state.isLocked) return;
		...
		const btnSetPrevActive = veT`<button class="ve-btn ve-btn-default ve-btn-xs" title="${I18nUtil.get("page.dmscreen.previous_turn")}"><span class="glyphicon glyphicon-step-backward"></span></button>`
			.vee.onn("click", () => this._viewRowsActive.pDoShiftActiveRow({direction: InitiativeTrackerConst.DIR_BACKWARDS}));
		const btnSetNextActive = veT`<button class="ve-btn ve-btn-default ve-btn-xs ve-mr-2" title="${I18nUtil.get("page.dmscreen.next_turn")}"><span class="glyphicon glyphicon-step-forward"></span></button>`
		...
		const iptRound = ComponentUiUtil.getIptInt(this, "round", 1, {min: 1})
			...
			.vee.tooltip(I18nUtil.get("page.dmscreen.round"));
		...
		const btnNetworking = veT`<button class="ve-btn ve-btn-primary ve-btn-xs ve-mr-2" title="配置玩家视图 (按住SHIFT打开&quot;标准&quot;视图配置)"><span class="glyphicon glyphicon-user"></span></button>`
			.vee.onn("click", evt => {
		...
		const btnLock = veT`<button class="ve-btn ve-btn-danger ve-btn-xs" title="${I18nUtil.get("page.dmscreen.lock_tracker")}"><span class="glyphicon glyphicon-lock"></span></button>`
		...
		this._addHookBase("isLocked", () => {
			btnLock
				...
				.vee.tooltip(this._state.isLocked ? I18nUtil.get("page.dmscreen.unlock_tracker") : I18nUtil.get("page.dmscreen.lock_tracker"));
		...
		const btnConfigure = veT`<button class="ve-btn ve-btn-default ve-btn-xs ve-mr-2" title="${I18nUtil.get("common.button.configure")} (${I18nUtil.get("page.dmscreen.shift_to_open_settings")})"><span class="glyphicon glyphicon-cog"></span></button>`
		...
		const btnLoad = veT`<button title="${I18nUtil.get("page.dmscreen.import_encounter_from_the_bestiary")}" class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable"><span class="glyphicon glyphicon-upload"></span></button>`
			.vee.onn("click", (evt) => {
				if (this._state.isLocked) return;
				ContextUtil.pOpenMenu(evt, menuImport);
			});
		const btnReset = veT`<button title="${I18nUtil.get("page.dmscreen.reset_tracker")}" class="ve-btn ve-btn-danger ve-btn-xs dm-init-lockable"><span class="glyphicon glyphicon-trash"></span></button>`
			.vee.onn("click", async () => {
				if (this._state.isLocked) return;
