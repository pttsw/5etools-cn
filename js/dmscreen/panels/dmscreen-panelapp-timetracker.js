...
class TimeTrackerBase extends TimeTrackerComponent {
	...
	static formatYearInfo (year, yearInfos, eraInfos, abbreviate) {
		return `第${year + 1}年${yearInfos.length ? ` (<span class="ve-italic">${yearInfos.map(it => it.name.escapeQuotes()).join("/")}</span>)` : ""}${eraInfos.length ? `, ${eraInfos.map(it => `${it.dayOfEra + 1} <span ${abbreviate ? `title="${it.name.escapeQuotes()}"` : ``}>${(abbreviate ? it.abbreviation : it.name).escapeQuotes()}</span>${abbreviate ? "" : ` (${it.abbreviation.escapeQuotes()})`}`).join("/")}` : ""}`;
	}
...
class TimeTrackerRoot extends TimeTrackerBase {
	...
	render (eleParent) {
		...
		const btnShowClock = veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2" title="时钟"><span class="glyphicon glyphicon-time"></span></button>`
			.vee.onn("click", () => this._state.tab = 0);
		const btnShowCalendar = veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-3" title="日历"><span class="glyphicon glyphicon-calendar"></span></button>`
			.vee.onn("click", () => this._state.tab = 1);
		const btnShowSettings = veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-3" title="设置"><span class="glyphicon glyphicon-cog"></span></button>`
		...
		const btnReset = veT`<button class="ve-btn ve-btn-xs ve-btn-danger" title="重置时钟/日历时间为第一天"><span class="glyphicon glyphicon-refresh"></span></button>`
			.vee.onn("click", async () => {
...
class TimeTrackerRoot_Clock extends TimeTrackerComponent {
	...
	render (eleParent, parent) {
		...
		const btnNextSunrise = veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="跳过到下一个日出前的时间。如果已经到凌晨了，则会跳到当天早上，否则调到转天早上。">下一个日出</button>`
			.vee.onn("click", () => {
		...
		const btnNextDay = veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="跳过到下一个凌晨前的时间。">下一天</button>`
		...
		const hookClock = () => {
			...
			if (seasonInfos.length) {
				...
				const dayNightHtml = seasonInfos.map(it => {
					...
					return `<b class="ve-mr-2">${isDay ? "白天" : "夜晚"}</b> <span class="small ve-muted">(${hoursToDayNight === 1 ? `开始不到1小时` : `距离日${isDay ? "落" : "出"}${hoursToDayNight - 1}个多小时`})</span>`;
				}).join("/");
		...
		const btnSubDay = veT`<button class="ve-btn ve-btn-xxs ve-btn-default dm-time__btn-day"  title="减一天(按住SHIFT减五天)">-</button>`
		...
		const btnAddDay = veT`<button class="ve-btn ve-btn-xxs ve-btn-default dm-time__btn-day" title="加一天(按住SHIFT加五天)">+</button>`
		...
		const btnAddHour = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--top" title="加一小时(按住SHIFT加5小时,按住CTRL加12小时)">+</button>`
		...
		const btnSubHour = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--bottom" title="减一小时(按住SHIFT减5小时,按住CTRL减12小时)">-</button>`
		...
		const btnAddMinute = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--top" title="加一分钟(按住SHIFT加5分钟,按住CTRL加15分钟,同时按住加30分钟)">+</button>`
		...
		const btnSubMinute = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--bottom" title="减一分钟(按住SHIFT减5分钟,按住CTRL减15分钟,同时按住减30分钟)">-</button>`
		...
		const btnAddSecond = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--top" title="加一秒(按住SHIFT加5秒,按住CTRL加15秒,同时按住加30秒)">+</button>`
		...
		const btnSubSecond = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-time dm-time__btn-time--bottom" title="减一秒(按住SHIFT减5秒,按住CTRL减15秒,同时按住减30秒)">-</button>`
		...
		const btnAddLongRest = veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="添加一次长休时间 (按住SHIFT改为减去)">长休</button>`
		...
		const btnAddShortRest = veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2" title="添加一次短休时间 (按住SHIFT改为减去)">短休</button>`
		...
		const btnAddTurn = veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="添加一个回合时间 (按住SHIFT改为减去)">回合</button>`
...
class TimeTrackerRoot_Clock_Weather extends TimeTrackerComponent {
	render (eleParent, parent) {
		...
		const btnRandomise = veT`<button class="ve-btn ve-btn-xxs ve-btn-default dm-time__btn-random-weather" title="随机生成天气 (按住SHIFT重新使用之前的设置)"><span class="fal fa-dice"></span></button>`
		...
		veT`<div class="ve-flex-col ve-w-100 ve-flex-vh-center">
			<div class="ve-flex-vh-center small ve-mb-1"><span class="ve-small-caps ve-mr-2">天气</span>${btnRandomise}</div>
			<div class="ve-mb-2">${btnTemperature}${btnPrecipitation}</div>

			<div class="ve-flex-col ve-flex-vh-center">
				<div class="small ve-small-caps">风向/风速</div>
...
class TimeTrackerRoot_Clock_RandomWeather extends BaseComponent {
	...
	render (eleModalInner, doClose) {
		...
		const btnOk = veT`<button class="ve-btn ve-btn-default">确认并随机生成天气</button>`
			.vee.onn("click", () => {
		...
		veT`<div class="ve-flex-col ve-w-100 ve-h-100">
			...
			<div class="ve-flex-v-center ve-mt-2">
				<h5 class="ve-mr-2">主导风向</h5>${btnWindDirection}
			</div>
			<div class="ve-flex-col">
				<h5>可选的风速</h5>
...
class TimeTrackerRoot_Calendar extends TimeTrackerComponent {
	...
	render (eleParent, parent) {
		...
		const btnBrowseMode = ComponentUiUtil.getBtnBool(
			...
			"isBrowseMode",
			{
				ele: veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="当启用此选项时，当前日历试图将被暂存，你可以随意浏览其他日期，当你浏览结束后，你可以禁用此选项以返回保存的视图。">浏览模式</button>`,
				fnHookPost: val => {
	...
	static getDateControls (parent, opts) {
		...
		const btnSubDay = opts.isHideDays ? null : veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一天(按住SHIFT减5天)">\u2212D</button>`
			.vee.onn("click", evt => doModTime(-1 * getTimeInfo().secsPerDay * (evt.shiftKey ? 5 : 1)));
		const btnAddDay = opts.isHideDays ? null : veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一天(按住SHIFT加5天)">D+</button>`
			.vee.onn("click", evt => doModTime(getTimeInfo().secsPerDay * (evt.shiftKey ? 5 : 1)));

		const btnSubWeek = opts.isHideWeeks ? null : veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一周(按住SHIFT减5周)">\u2212W</button>`
			.vee.onn("click", evt => doModTime(-1 * getTimeInfo().secsPerWeek * (evt.shiftKey ? 5 : 1)));
		const btnAddWeek = opts.isHideWeeks ? null : veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一周(按住SHIFT加5周)">W+</button>`
		...
		const btnSubMonth = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一个月(按住SHIFT减五个月)">\u2212M</button>`
			.vee.onn("click", evt => doModMonths(evt.shiftKey ? -5 : -1));
		const btnAddMonth = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一个月(按住SHIFT加五个月)">M+</button>`
			.vee.onn("click", evt => doModMonths(evt.shiftKey ? 5 : 1));

		const btnSubYear = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust"  title="减一年(按住SHIFT减五年)">\u2212Y</button>`
			.vee.onn("click", evt => doModTime(-1 * getTimeInfo().secsPerYear * (evt.shiftKey ? 5 : 1)));
		const btnAddYear = veT`<button class="ve-btn ve-btn-xs ve-btn-default dm-time__btn-date-adjust" title="加一年(按住SHIFT加五年)">Y+</button>`
	...
	render (eleParent, parent) {
		...
		const sectMonths = veT`<div class="ve-no-shrink ve-w-100">
			<div class="ve-flex ve-w-100 ve-mb-1 ve-mt-1">
				<div class="ve-w-100 ve-flex-v-center">名称</div>
...
class TimeTrackerRoot_Settings_Event extends TimeTrackerComponent {
	...
	doOpenEditModal (overlayColor = "transparent") {
		...
		const btnOk = veT`<button class="ve-btn ve-btn-default" data-i18n='common.button.save'></button>`
			.vee.onn("click", () => doClose(true));
