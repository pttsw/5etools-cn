...
export class MoneyConverter extends DmScreenPanelAppBase {
	...
	_getPanelElement (board, state) {
		...
		const buildCurrencySelect = (isOutput) => ee`<select class="form-control input-sm p-2">${isOutput ? `<option value="-1">(未指定)</option>` : ""}${CURRENCY.map((c, i) => `<option value="${i}">${c.n}</option>`).join("")}</select>`;
