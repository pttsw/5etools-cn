...
export class MoneyConverter extends DmScreenPanelAppBase {
	...
	_getPanelElement (board, state) {
		...
		const buildCurrencySelect = (isOutput) => ee`<select class="ve-form-control ve-input-sm ve-p-2">${isOutput ? `<option value="-1">(未指定)</option>` : ""}${CURRENCY.map((c, i) => `<option value="${i}">${c.n}</option>`).join("")}</select>`;

		const addRow = (currency, count) => {
			const eleRow = ee`<div class="dm-money__row"></div>`.appendTo(this._wrpRows);
			const iptCount = ee`<input type="number" step="1" placeholder="数量" class="ve-form-control ve-input-sm">`.appendTo(eleRow).onn("change", doUpdate);
		...
		const btnAddRow = ee`<button class="ve-btn ve-btn-primary ve-btn-sm" title="添加一行"><span class="glyphicon glyphicon-plus"></span></button>`
			.appendTo(wrpBtnAddSettings)
			.onn("click", () => {
				addRow();
				doUpdate();
			});
		const btnSettings = ee`<button class="ve-btn ve-btn-default ve-btn-sm" title="设置"><span class="glyphicon glyphicon-cog"></span></button>`
			.appendTo(wrpBtnAddSettings)
			.onn("click", () => {
		...
		this._iptSplit = ee`<input type="number" min="1" step="1" placeholder="分成...份" class="ve-form-control ve-input-sm">`.appendTo(wrpCtrlRhs).onn("change", doUpdate);
