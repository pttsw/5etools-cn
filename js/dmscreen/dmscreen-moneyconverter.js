...
export class MoneyConverter extends DmScreenPanelAppBase {
	...
	_getPanelElement (board, state) {
		...
		const buildCurrencySelect = (isOutput) => ee`<select class="form-control input-sm" style="padding: 5px">${isOutput ? `<option value="-1">(未指定)</option>` : ""}${CURRENCY.map((c, i) => `<option value="${i}">${c.n}</option>`).join("")}</select>`;

		const addRow = (currency, count) => {
			const eleRow = ee`<div class="dm-money__row"></div>`.appendTo(this._wrpRows);
			const iptCount = ee`<input type="number" step="1" placeholder="数量" class="form-control input-sm">`.appendTo(eleRow).onn("change", doUpdate);
		...
		this._iptSplit = ee`<input type="number" min="1" step="1" placeholder="分成...份" class="form-control input-sm">`.appendTo(wrpCtrlRhs).onn("change", doUpdate);
