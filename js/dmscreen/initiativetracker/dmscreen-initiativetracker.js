...
export class InitiativeTracker extends BaseComponent {
	...
	_render_getWrpButtonsSort () {
		const btnSortAlpha = ee`<button title="${I18nUtil.get("page.dmscreen.sort_alphabetically")}" class="ve-btn ve-btn-default ve-btn-xs"><span class="glyphicon glyphicon-sort-by-alphabet"></span></button>`
			.onn("click", () => {
		...
		const btnSortNumber = ee`<button title="${I18nUtil.get("page.dmscreen.sort_numerically")}" class="ve-btn ve-btn-default ve-btn-xs"><span class="glyphicon glyphicon-sort-by-order"></span></button>`
			.onn("click", () => {
	...
	_render_getWrpFooter ({wrpTracker, doUpdateExternalStates}) {
		const btnAdd = ee`<button class="ve-btn ve-btn-primary ve-btn-xs dm-init-lockable" title="${I18nUtil.get("page.dmscreen.add_player")}"><span class="glyphicon glyphicon-plus"></span></button>`
			.onn("click", async () => {
		...
		const btnAddMonster = ee`<button class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable mr-2" title="${I18nUtil.get("page.dmscreen.add_creature")}"><span class="glyphicon glyphicon-print"></span></button>`
			.onn("click", async () => {
				if (this._state.isLocked) return;
		...
		const btnSetPrevActive = ee`<button class="ve-btn ve-btn-default ve-btn-xs" title="${I18nUtil.get("page.dmscreen.previous_turn")}"><span class="glyphicon glyphicon-step-backward"></span></button>`
			.onn("click", () => this._viewRowsActive.pDoShiftActiveRow({direction: InitiativeTrackerConst.DIR_BACKWARDS}));
		const btnSetNextActive = ee`<button class="ve-btn ve-btn-default ve-btn-xs mr-2" title="${I18nUtil.get("page.dmscreen.next_turn")}"><span class="glyphicon glyphicon-step-forward"></span></button>`
		...
		const iptRound = ComponentUiUtil.getIptInt(this, "round", 1, {min: 1})
			...
			.tooltip(I18nUtil.get("page.dmscreen.round"));
		...
		const btnNetworking = ee`<button class="ve-btn ve-btn-primary ve-btn-xs mr-2" title="${I18nUtil.get("page.dmscreen.player_view")} (${I18nUtil.get("page.dmscreen.shift_to_open_standard_view")})"><span class="glyphicon glyphicon-user"></span></button>`
			.onn("click", evt => {
		...
		const btnLock = ee`<button class="ve-btn ve-btn-danger ve-btn-xs" title="${I18nUtil.get("page.dmscreen.lock_tracker")}"><span class="glyphicon glyphicon-lock"></span></button>`
		...
		this._addHookBase("isLocked", () => {
			btnLock
				...
				.tooltip(this._state.isLocked ? I18nUtil.get("page.dmscreen.unlock_tracker") : I18nUtil.get("page.dmscreen.lock_tracker"));
		...
		const btnConfigure = ee`<button class="ve-btn ve-btn-default ve-btn-xs mr-2" title="${I18nUtil.get("common.button.configure")} (${I18nUtil.get("page.dmscreen.shift_to_open_settings")})"><span class="glyphicon glyphicon-cog"></span></button>`
			.onn("click", async evt => {
		...
		const btnLoad = ee`<button title="${I18nUtil.get("page.dmscreen.import_encounter_from_the_bestiary")}" class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable"><span class="glyphicon glyphicon-upload"></span></button>`
			.onn("click", (evt) => {
				if (this._state.isLocked) return;
				ContextUtil.pOpenMenu(evt, menuImport);
			});
		const btnReset = ee`<button title="${I18nUtil.get("page.dmscreen.reset_tracker")}" class="ve-btn ve-btn-danger ve-btn-xs dm-init-lockable"><span class="glyphicon glyphicon-trash"></span></button>`
			.onn("click", async () => {
				if (this._state.isLocked) return;
