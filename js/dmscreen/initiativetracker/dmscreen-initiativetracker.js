...
export class InitiativeTracker extends BaseComponent {
	...
	_render_getWrpFooter ({wrpTracker, doUpdateExternalStates}) {
		...
		const btnAddMonster = ee`<button class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable ve-mr-2" title="${I18nUtil.get("page.dmscreen.add_creature")}"><span class="glyphicon glyphicon-print"></span></button>`
			.onn("click", async () => {
				if (this._state.isLocked) return;
		...
		const btnSetNextActive = ee`<button class="ve-btn ve-btn-default ve-btn-xs ve-mr-2" title="${I18nUtil.get("page.dmscreen.next_turn")}"><span class="glyphicon glyphicon-step-forward"></span></button>`
		...
		const btnNetworking = ee`<button class="ve-btn ve-btn-primary ve-btn-xs ve-mr-2" title="配置玩家视图 (按住SHIFT打开&quot;标准&quot;视图配置)"><span class="glyphicon glyphicon-user"></span></button>`
			.onn("click", evt => {
		...
		const btnConfigure = ee`<button class="ve-btn ve-btn-default ve-btn-xs ve-mr-2" title="${I18nUtil.get("common.button.configure")} (${I18nUtil.get("page.dmscreen.shift_to_open_settings")})"><span class="glyphicon glyphicon-cog"></span></button>`
			.onn("click", async evt => {
