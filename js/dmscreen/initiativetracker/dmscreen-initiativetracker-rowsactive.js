...
class _RenderableCollectionRowDataActive extends RenderableCollectionRowDataBase {
	...
	_pPopulateRow_monster ({comp, wrpLhs, isMon, mon, fluff}) {
		...
		const btnRename = ee`<button class="ve-btn ve-btn-default ve-btn-xs dm-init-lockable dm-init__btn-creature" title="重命名(按住SHIFT来重置)" tabindex="-1"><span class="glyphicon glyphicon-pencil"></span></button>`
			.onn("click", async evt => {
		...
		const btnDuplicate = ee`<button class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable dm-init__btn-creature" title="添加另一个(按住SHIFT来为新的掷骰)" tabindex="-1"><span class="glyphicon glyphicon-plus"></span></button>`
			.onn("click", async (evt) => {
				if (this._comp._state.isLocked) return;
	...
	_pPopulateRow_conditions ({comp, wrpLhs}) {
		const btnAddCond = ee`<button class="ve-btn ve-btn-warning ve-btn-xs dm-init__row-btn dm-init__row-btn-flag" title="${I18nUtil.get("page.dmscreen.add_contition")}" tabindex="-1"><span class="glyphicon glyphicon-flag"></span></button>`
			.onn("click", async () => {
