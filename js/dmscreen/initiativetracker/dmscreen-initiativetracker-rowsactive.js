...
class _RenderableCollectionRowDataActive extends RenderableCollectionRowDataBase {
	...
	_pPopulateRow_monster ({comp, $wrpLhs, isMon, mon, fluff}) {
		...
		const $btnRename = $(`<button class="ve-btn ve-btn-default ve-btn-xs dm-init-lockable dm-init__btn-creature" title="重命名(按住SHIFT来重置)" tabindex="-1"><span class="glyphicon glyphicon-pencil"></span></button>`)
			.click(async evt => {
		...
		const $btnDuplicate = $(`<button class="ve-btn ve-btn-success ve-btn-xs dm-init-lockable dm-init__btn-creature" title="添加另一个(按住SHIFT来为新的掷骰)" tabindex="-1"><span class="glyphicon glyphicon-plus"></span></button>`)
			.click(async (evt) => {
				if (this._comp._state.isLocked) return;
