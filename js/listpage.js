...
class ListPage {
	...
	_renderListShowHide ({$wrpContent, $wrpList, $btnReset}) {
		const $btnHideSearch = $(`<button class="ve-btn ve-btn-default" title="Hide Search Bar and Entry List">隐藏</button>`);
		$btnReset.before($btnHideSearch);

		const $btnShowSearch = $(`<button class="ve-btn ve-btn-block ve-btn-default ve-btn-xs" type="button">查看列表</button>`);
	...
	_renderListFeelingLucky ({isCompact, $btnReset, isScrollablePage = false}) {
		const btnRoll = ee`<button class="ve-btn ve-btn-default ${isCompact ? "px-2" : ""}" title="随便选一个?"><span class="glyphicon glyphicon-random"></span></button>`;
	...
	async _pHandleClick_exportAsImage ({evt, isFast, $eleCopyEffect}) {
		...
		const $btnCpy = $(`<button class="ve-btn ve-btn-default ve-btn-xs" title="SHIFT to Copy and Close">复制</button>`)
			.on("click", async evt => {
		...
		const $btnSave = $(`<button class="ve-btn ve-btn-default ve-btn-xs" title="SHIFT to Save and Close">保存</button>`)
			.on("click", async evt => {
				this._pHandleClick_exportAsImage_mutOptions({$ele: $cpy, optsDomToImage});
