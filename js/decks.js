...
class DecksPage extends ListPage {
	...
	async _handleClick_pDoOpenSpread (ent, btnSpread, {isSkipAnimation = false} = {}) {
		try {
			...
			const btnRedraw = veT`<button class="ve-btn ve-btn-primary ve-btn-sm ve-no-shrink" title="Draw Spread (CTRL to Skip Animation)">抽卡</button>`
	...
	_renderStats_doBuildStatsTab ({ent}) {
		...
		const btnSpread = ent.spreads?.length
			? veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-bb-0 ve-bbr-0 ve-bbl-0" title="解读牌阵 (CTRL to Skip Animation)"><i class="fas fa-fw fa-layer-group"></i></button>`
