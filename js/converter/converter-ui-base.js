...
export class ConverterUiBase extends BaseComponent {
	...
	_renderSettings_source ({compParent, wrpSettings}) {
		...
		const selSource = veT`<select class="ve-form-control ve-input-xs ve-br-0 ve-w-120p ve-text-clip-ellipsis"><option value="">(无)</option></select>`
		...
		const btnSourceEdit = veT`<button class="ve-btn ve-btn-default ve-btn-xs" title="编辑当前来源"><span class="glyphicon glyphicon-pencil"></span></button>`
			.vee.onn("click", () => {
		...
		const btnSourceAdd = veT`<button class="ve-btn ve-btn-default ve-btn-xs" title="新增来源"><span class="glyphicon glyphicon-plus"></span></button>`
			.vee.onn("click", () => {
	...
	_renderSettings_page ({wrpSettings, fnsCleanup}) {
		...
		const dispPage = veT`<div class="ve-mr-2 ve-help">页码</div>`;
	...
	_renderSettings_samples ({wrpModesSamples}) {
		const btnsSamples = this._modes.map(mode => {
			return veT`<button class="ve-btn ve-btn-xs ve-btn-default">范例${ConverterUiBase._getDisplayMode(mode)}</button>`
				.vee.onn("click", () => {
	...
	_renderSettingsModal_converterOptions ({eleParent}) {
		...
		veT`<div class="ve-flex-col">
			...
			${this._hasPageNumbers ? veT`<label class="ve-split-v-center ve-w-100" title="Should the output include the page field?">
				<span class="ve-w-66 ve-no-shrink ve-mr-2 ve-flex-v-center">增加页码</span>
				${cbAddPageNumber}
			</label>` : null}
