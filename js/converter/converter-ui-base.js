...
export class ConverterUiBase extends BaseComponent {
	...
	_renderSidebarConverterOptionsPart (parent, wrpSidebar) {
		...
		if (hasModes) {
			const selMode = ComponentUiUtil.getSelEnum(this, "mode", {values: this._modes, html: `<select class="ve-form-control ve-input-xs select-inline"></select>`, fnDisplay: it => `从${ConverterUiBase._getDisplayMode(it)}转换`});
	...
	_renderSidebarPagePart (parent, wrpSidebar) {
		...
		ee`<div class="ve-w-100 ve-split-v-center">
			<div class="sidemenu__row__label ve-mr-2 ve-help" title="Note that a line of the form &quot;PAGE=&lt;page number&gt;&quot; in the Input will set the page in the Output, ignoring any value set here. This is especially useful when parsing multiple inputs delimited by a separator.">页数</div>
	...
	_renderSidebarSourcePart (parent, wrpSidebar) {
		...
		ee`<div class="ve-w-100 ve-mb-2 ve-split-v-center"><div class="sidemenu__row__label ve-mr-2">来源</div>${selSource}</div>`.appendTo(wrpSidebar);
