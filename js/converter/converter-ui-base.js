...
export class ConverterUiBase extends BaseComponent {
	...
	_renderSettings_source ({compParent, wrpSettings}) {
		...
		const selSource = ee`<select class="ve-form-control ve-input-xs ve-br-0 ve-w-120p ve-text-clip-ellipsis"><option value="">(无)</option></select>`
	...
	_renderSettings_page ({wrpSettings, fnsCleanup}) {
		...
		const dispPage = ee`<div class="ve-mr-2 ve-help">页码</div>`;
