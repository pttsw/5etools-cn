...
export class ManageBrewUi {
	...
	_getBtnDeleteAll (rdState) {
		...
		return veT`<button class="ve-btn ve-btn-danger" title="SHIFT to also delete all ${brewUtilOther.DISPLAY_NAME.toTitleCase()}">${I18nUtil.get("common.button.delete_all")}</button>`
	...
	_getBtnPullAll (rdState) {
		const btn = veT`<button class="ve-btn ve-btn-default ve-w-80p">${I18nUtil.get("common.button.update_all")}</button>`
			.vee.addClass(this._isModal ? "ve-btn-xs" : "ve-btn-sm")
			.vee.onn("click", async () => {
	...
	async pRender (wrp, {rdState = null} = {}) {
		...
		const btnGet = veT`<button class="ve-btn ${this._brewUtil.STYLE_BTN} ve-btn-sm">${I18nUtil.get("common.button.get")} ${this._brewUtil.DISPLAY_NAME.toTitleCase()}</button>`
			.vee.onn("click", () => this._pHandleClick_btnGetBrew(rdState));

		const btnCustomUrl = veT`<button class="ve-btn ${this._brewUtil.STYLE_BTN} ve-btn-sm ve-px-2" title="设置自定义仓库URL"><span class="glyphicon glyphicon-cog"></span></button>`
			.vee.onn("click", () => this._pHandleClick_btnSetCustomRepo());

		const btnLoadPartnered = veT`<button class="ve-btn ve-btn-default ve-btn-sm">加载所有合作内容</button>`
			.vee.onn("click", () => this._pHandleClick_btnLoadPartnered(rdState));

		const btnLoadFromFile = veT`<button class="ve-btn ve-btn-default ve-btn-sm">${I18nUtil.get("common.button.load_from_file")}</button>`
			.vee.onn("click", () => this._pHandleClick_btnLoadFromFile(rdState));

		const btnLoadFromUrl = veT`<button class="ve-btn ve-btn-default ve-btn-sm">${I18nUtil.get("common.button.load_from_url")}</button>`
		...
		const btnSaveToUrl = veT`<button class="ve-btn ve-btn-default ve-btn-sm" title="Note that this does not include &quot;Editable&quot; or &quot;Local&quot; content.">${I18nUtil.get("common.button.export_list_as_url")}</button>`
			.vee.onn("click", async evt => {
		...
		const wrpBtns = veT`<div class="ve-flex-v-center ve-no-shrink ve-mobile-sm__flex-col">
			...
			<div class="ve-flex-v-center">
				<a href="${this._brewUtil.URL_REPO_DEFAULT}" class="ve-flex-v-center" target="_blank" rel="noopener noreferrer"><button class="ve-btn ve-btn-default ve-btn-sm ve-mr-2">${I18nUtil.get("common.button.browse_source_repository")}</button></a>
	...
	async _pRender_pBrewList (rdState) {
		...
		const btnMass = veT`<button class="ve-btn ve-btn-default ve-bbl-0 ve-self-flex-stretch">${I18nUtil.get("common.button.mass")}...</button>`
			.vee.onn("click", evt => this._pHandleClick_btnListMass({evt, rdState}));
		const iptSearch = veT`<input type="search" class="search manbrew__search ve-form-control ve-bbr-0" placeholder="${I18nUtil.get("common.button.search")} ${this._brewUtil.DISPLAY_NAME}...">`;
		...
		const rowsSubMetas = sources
			.map(brewSource => {
				...
				const btnConvertedBy = veE({
					...
					children: [
						veE({tag: "span", clazz: "ve-mobile-sm__hidden", txt: "查看编辑"}),
						veE({tag: "span", clazz: "ve-mobile-sm__visible", txt: "编辑", title: "查看编辑"}),
					],
					click: () => {
				...
				const lnkUrl = brewSource.url
					? veE({
						...
						txt: I18nUtil.get("page.manageprelease.view_source"),
					})
					: veE({
						tag: "span",
						clazz: "ve-col-2 ve-text-center",
