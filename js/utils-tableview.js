...
class UtilsTableview {
	...
	static show ({title, entities, colTransforms, additionalData, sorter}) {
		...
		const btnCsv = veT`<button class="ve-btn ve-btn-primary">下载CSV</button>`.vee.onn("click", () => {
			DataUtil.userDownloadText(`${title}.csv`, this._getAsCsv({colTransforms, rdState}));
		});

		const btnCopy = veT`<button class="ve-btn ve-btn-primary">复制CSV到剪贴板</button>`.vee.onn("click", async () => {
