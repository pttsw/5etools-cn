...
class UtilsTableview {
	...
	static show ({title, entities, colTransforms, additionalData, sorter}) {
		...
		const btnCsv = ee`<button class="ve-btn ve-btn-primary">下载CSV</button>`.onn("click", () => {
			DataUtil.userDownloadText(`${title}.csv`, this._getAsCsv({colTransforms, rdState}));
		});

		const btnCopy = ee`<button class="ve-btn ve-btn-primary">复制CSV到剪贴板</button>`.onn("click", async () => {
