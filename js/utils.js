...
globalThis.DEPLOYED_IMG_ROOT = "/img/";
...
globalThis.VeCt = class {
	...
	static URL_BREW = `https://github.com/tjliqy/homebrew/`;
	static URL_ROOT_BREW = `https://homebrew.kiwee.top/`; // N.b. must end with a slash
	static URL_ROOT_BREW_IMG = `${URL_PROXY}https://raw.githubusercontent.com/tjliqy/homebrew-img/main/`; // N.b. must end with a slash
	static URL_PRERELEASE = `https://github.com/tjliqy/unearthed-arcana`;
	static URL_ROOT_PRERELEASE = `https://ua.kiwee.top/`; // As above

	static STR_NO_ATTUNEMENT = "无需同调";
...
UrlUtil.PG_TO_NAME[UrlUtil.PG_BESTIARY] = "怪物图鉴";
UrlUtil.PG_TO_NAME[UrlUtil.PG_SPELLS] = "法术";
UrlUtil.PG_TO_NAME[UrlUtil.PG_BACKGROUNDS] = "背景";
UrlUtil.PG_TO_NAME[UrlUtil.PG_ITEMS] = "物品";
UrlUtil.PG_TO_NAME[UrlUtil.PG_CLASSES] = "职业";
UrlUtil.PG_TO_NAME[UrlUtil.PG_CONDITIONS_DISEASES] = "状态 & 疾病";
UrlUtil.PG_TO_NAME[UrlUtil.PG_FEATS] = "专长";
UrlUtil.PG_TO_NAME[UrlUtil.PG_OPT_FEATURES] = "职业能力选项";
UrlUtil.PG_TO_NAME[UrlUtil.PG_PSIONICS] = "灵能";
UrlUtil.PG_TO_NAME[UrlUtil.PG_RACES] = "种族";
UrlUtil.PG_TO_NAME[UrlUtil.PG_REWARDS] = "其他奖励";
UrlUtil.PG_TO_NAME[UrlUtil.PG_VARIANTRULES] = "术语汇编";
UrlUtil.PG_TO_NAME[UrlUtil.PG_ADVENTURES] = "冒险";
UrlUtil.PG_TO_NAME[UrlUtil.PG_BOOKS] = "书籍";
UrlUtil.PG_TO_NAME[UrlUtil.PG_DEITIES] = "神祇";
...
UrlUtil.PG_TO_NAME[UrlUtil.PG_DEMO_RENDER] = "渲染器Demo";
UrlUtil.PG_TO_NAME[UrlUtil.PG_TABLES] = "表格";
UrlUtil.PG_TO_NAME[UrlUtil.PG_VEHICLES] = "载具";
// UrlUtil.PG_TO_NAME[UrlUtil.PG_CHARACTERS] = "";
UrlUtil.PG_TO_NAME[UrlUtil.PG_ACTIONS] = "动作";
UrlUtil.PG_TO_NAME[UrlUtil.PG_LANGUAGES] = "语言";
UrlUtil.PG_TO_NAME[UrlUtil.PG_STATGEN] = "属性生成器";
UrlUtil.PG_TO_NAME[UrlUtil.PG_LIFEGEN] = "这是你的人生";
UrlUtil.PG_TO_NAME[UrlUtil.PG_NAMES] = "名字";
UrlUtil.PG_TO_NAME[UrlUtil.PG_DM_SCREEN] = "DM帷幕";
UrlUtil.PG_TO_NAME[UrlUtil.PG_CR_CALCULATOR] = "CR 计算器";
UrlUtil.PG_TO_NAME[UrlUtil.PG_ENCOUNTERGEN] = "遭遇生成器";
UrlUtil.PG_TO_NAME[UrlUtil.PG_LOOTGEN] = "战利品生成器";
UrlUtil.PG_TO_NAME[UrlUtil.PG_TEXT_CONVERTER] = "文本转换器";
...
UrlUtil.PG_TO_NAME[UrlUtil.PG_TRAP_FEATURES] = "陷阱特性";
UrlUtil.PG_TO_NAME[UrlUtil.PG_MAPS] = "地图";
UrlUtil.PG_TO_NAME[UrlUtil.PG_DECKS] = "牌组";
UrlUtil.PG_TO_NAME[UrlUtil.PG_BASTIONS] = "据点";
...
globalThis.RollerUtil = class {
	...
	static REGEX_CN_DAMAGE_DICE = /(?<average>\d+)(?<prefix> ?[(（](?:{@dice |{@damage ))(?<diceExp>[-+0-9d ]*)(?<suffix>}[)）](?:\s*(?:\+|加上?)\s*(?:你的)?法术环阶)? ?[a-z\u4e00-\u9fa5]+( ?[(（][-a-zA-Z0-9\u4e00-\u9fa5 ]+[)）])?( ?或 ?[a-z\u4e00-\u9fa5]+( ?[(（][-a-zA-Z0-9\u4e00-\u9fa5 ]+[)）])?)?伤害)/gi;
...
class BookModeViewBase {
	...
	async _pGetWrpControls ({wrpContent}) {
		...
		const selColumns = veT`<select class="ve-form-control ve-input-sm">
			<option value="2">两列 (同书中样式)</option>
		...
		const wrpPrint = veT`<div class="ve-w-100 ve-flex">
			<div class="ve-flex-vh-center"><div class="ve-mr-2 ve-no-wrap ve-help-subtle" title="Applied when printing the page.">打印列数：</div>${selColumns}</div>
	...
	_getBtnNoneVisibleClose () {
		return veT`<button class="ve-btn ve-btn-default">关闭</button>`
			.vee.onn("click", () => this.setStateClosed());
	}
