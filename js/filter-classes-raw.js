...
export class ModalFilterClasses extends ModalFilterBase {
	...
	async pPreloadHidden (eleModalInner) {
		...
		const ovlLoading = ee`<div class="w-100 h-100 ve-flex-vh-center"><i class="dnd-font ve-muted">加载中...</i></div>`.appendTo(eleModalInner);

		const iptSearch = ee`<input class="form-control h-100" type="search" placeholder="搜索...">`;
		...
		const wrpFormHeaders = ee`<div class="input-group input-group--bottom ve-flex no-shrink">
			...
			<button class="ve-col-2 pr-0 sort ve-btn ve-btn-default ve-btn-xs ve-grow" data-sort="source">来源</button>
		</div>`;
