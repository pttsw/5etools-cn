...
export class ModalFilterClasses extends ModalFilterBase {
	...
	async pPreloadHidden (eleModalInner) {
		...
		const ovlLoading = veT`<div class="ve-w-100 ve-h-100 ve-flex-vh-center"><i class="ve-dnd-font ve-muted">加载中...</i></div>`.vee.appendTo(eleModalInner);

		const iptSearch = veT`<input class="ve-form-control ve-h-100" type="search" placeholder="搜索...">`;
		...
		const wrpFormHeaders = veT`<div class="ve-input-group ve-input-group--bottom ve-flex ve-no-shrink">
			...
			<button class="ve-col-9 sort ve-btn ve-btn-default ve-btn-xs" data-sort="name">名称</button>
			<button class="ve-col-2 ve-pr-0 sort ve-btn ve-btn-default ve-btn-xs ve-grow" data-sort="source">来源</button>
		</div>`;
		...
		const btnConfirm = veT`<button class="ve-btn ve-btn-default">确认</button>`;
