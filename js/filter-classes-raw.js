...
export class ModalFilterClasses extends ModalFilterBase {
	...
	async pPreloadHidden (eleModalInner) {
		...
		const ovlLoading = ee`<div class="ve-w-100 ve-h-100 ve-flex-vh-center"><i class="ve-dnd-font ve-muted">加载中...</i></div>`.appendTo(eleModalInner);

		const iptSearch = ee`<input class="ve-form-control ve-h-100" type="search" placeholder="搜索...">`;
		...
		const btnConfirm = ee`<button class="ve-btn ve-btn-default">确认</button>`;
