...
export class BookUtil {
	...
	static _showBookContent_renderNavButtons ({isTop, ixChapter, bookId, data}) {
		...
		if (BookUtil.referenceId) {
			btnPrev = veT`<button class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print"><span class="glyphicon glyphicon-chevron-left"></span>上一章</button>`
				.vee.onn("click", () => this._showBookContent_goToPage({mod: -1, bookId, ixChapter}));
		} else {
			btnPrev = veT`<a href="#${this._showBookContent_goToPage({mod: -1, isGetHref: true, bookId, ixChapter})}" class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print"><span class="glyphicon glyphicon-chevron-left"></span>上一章</a>`
		...
		let btnNext;
		if (BookUtil.referenceId) {
			btnNext = veT`<button class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print">下一章<span class="glyphicon glyphicon-chevron-right"></span></button>`
				.vee.onn("click", () => this._showBookContent_goToPage({mod: 1, bookId, ixChapter}));
		} else {
			btnNext = veT`<a href="#${this._showBookContent_goToPage({mod: 1, isGetHref: true, bookId, ixChapter})}" class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print">下一章<span class="glyphicon glyphicon-chevron-right"></span></a>`
				.vee.onn("click", () => MiscUtil.scrollPageTop());
		}
		btnNext
			.vee.toggle(showNxt)
	...
	static _showBookContent_renderNavButtons_top ({bookId, wrpControls}) {
		...
		const btnEntireBook = veT`<a href="${href}" class="ve-btn ve-btn-xs ve-btn-default no-print ${~this.curRender.chapter ? "" : "ve-active"}" title="Warning: Slow">查看完整${Parser.getPropDisplayName(BookUtil.contentType)}</a>`;
		...
		const btnMenu = veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="其他选项"><span class="glyphicon glyphicon-option-vertical"></span></button>`
	...
	static _showBookContent_renderNavButtons_bottom ({bookId, wrpControls}) {
		veT`<button class="ve-btn ve-btn-xs ve-btn-default no-print">回到顶端</button>`
			.vee.onn("click", () => MiscUtil.scrollPageTop())
			.vee.appendTo(wrpControls);
	}
	...
	static async _booksHashChange_pHandleFound ({fromIndex, homebrewData, bookId, hashParts, wrpContents, isNewBook}) {
		...
		veEs(`#page__subtitle`).vee.html(I18nUtil.get("page.book.page_subtitle"));
