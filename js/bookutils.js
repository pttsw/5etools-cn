...
export class BookUtil {
	...
	static _showBookContent_renderNavButtons ({isTop, ixChapter, bookId, data}) {
		...
		if (BookUtil.referenceId) {
			$btnPrev = $(`<button class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print"><span class="glyphicon glyphicon-chevron-left"></span>上一章</button>`)
				.click(() => this._showBookContent_goToPage({mod: -1, bookId, ixChapter}));
		} else {
			$btnPrev = $(`<a href="#${this._showBookContent_goToPage({mod: -1, isGetHref: true, bookId, ixChapter})}" class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print"><span class="glyphicon glyphicon-chevron-left"></span>上一章</a>`)
		...
		let $btnNext;
		if (BookUtil.referenceId) {
			$btnNext = $(`<button class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print">下一章<span class="glyphicon glyphicon-chevron-right"></span></button>`)
				.click(() => this._showBookContent_goToPage({mod: 1, bookId, ixChapter}));
		} else {
			$btnNext = $(`<a href="#${this._showBookContent_goToPage({mod: 1, isGetHref: true, bookId, ixChapter})}" class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print">下一章<span class="glyphicon glyphicon-chevron-right"></span></a>`)
				.click(() => MiscUtil.scrollPageTop());
		}
		$btnNext
			.toggle(showNxt)
	...
	static _showBookContent_renderNavButtons_top ({bookId, $wrpControls}) {
		...
		const $btnEntireBook = $(`<a href="${href}" class="ve-btn ve-btn-xs ve-btn-default no-print ${~this.curRender.chapter ? "" : "active"}" title="Warning: Slow">查看完整${this.contentType.uppercaseFirst()}</a>`);
	...
	static _showBookContent_renderNavButtons_bottom ({bookId, $wrpControls}) {
		$(`<button class="ve-btn ve-btn-xs ve-btn-default no-print">回到顶端</button>`).click(() => MiscUtil.scrollPageTop()).appendTo($wrpControls);
	}
