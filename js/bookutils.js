...
export class BookUtil {
	...
	static _showBookContent_renderNavButtons ({isTop, ixChapter, bookId, data}) {
		...
		if (BookUtil.referenceId) {
			btnPrev = ee`<button class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print"><span class="glyphicon glyphicon-chevron-left"></span>上一章</button>`
				.onn("click", () => this._showBookContent_goToPage({mod: -1, bookId, ixChapter}));
		} else {
			btnPrev = ee`<a href="#${this._showBookContent_goToPage({mod: -1, isGetHref: true, bookId, ixChapter})}" class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print"><span class="glyphicon glyphicon-chevron-left"></span>上一章</a>`
		...
		let btnNext;
		if (BookUtil.referenceId) {
			btnNext = ee`<button class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print">下一章<span class="glyphicon glyphicon-chevron-right"></span></button>`
				.onn("click", () => this._showBookContent_goToPage({mod: 1, bookId, ixChapter}));
		} else {
			btnNext = ee`<a href="#${this._showBookContent_goToPage({mod: 1, isGetHref: true, bookId, ixChapter})}" class="ve-btn ve-btn-xs ve-btn-default bk__nav-head-foot-item no-print">下一章<span class="glyphicon glyphicon-chevron-right"></span></a>`
				.onn("click", () => MiscUtil.scrollPageTop());
		}
		btnNext
			.toggleVe(showNxt)
	...
	static _showBookContent_renderNavButtons_top ({bookId, wrpControls}) {
		...
		const btnEntireBook = ee`<a href="${href}" class="ve-btn ve-btn-xs ve-btn-default no-print ${~this.curRender.chapter ? "" : "active"}" title="Warning: Slow">查看完整${this.contentType.uppercaseFirst()}</a>`;
	...
	static _showBookContent_renderNavButtons_bottom ({bookId, wrpControls}) {
		ee`<button class="ve-btn ve-btn-xs ve-btn-default no-print">回到顶端</button>`
			.onn("click", () => MiscUtil.scrollPageTop())
			.appendTo(wrpControls);
	}
	...
	static async _booksHashChange_pHandleFound ({fromIndex, homebrewData, bookId, hashParts, wrpContents, isNewBook}) {
		...
		es(`#page__subtitle`).html(I18nUtil.get("page.book.page_subtitle"));
		...
		return ee`<div class="contents-item" data-bookid="${UrlUtil.encodeForHash(book.id)}">
			<div class="bk__contents-header">
				...
				<div class="ve-flex-v-center">
					<a href="${this._getHrefShowAll(book.id)}" class="bk__contents_show_all px-2 py-1p ve-flex-v-center lst__wrp-cells lst__row-inner" title="查看完整${BookUtil.contentType.uppercaseFirst()} (Warning: Slow)">
