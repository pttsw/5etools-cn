...
class SearchWidget {
		...
		if (resultCount) {
			...
			if (resultCount > UiUtil.SEARCH_RESULTS_CAP) {
				...
				if (I18nUtil.LANGUAGES_INDEX === "zh_CN") {
					this._wrpResults.appends(`<div class="ui-search__row ui-search__row--readonly">...${diff}条结果被隐藏，请调整搜索条件。</div>`);
				} else {
	...
	static _showLoadingModal () {
		...
		ee`<div class="ve-flex-vh-center w-100 h-100"><span class="dnd-font italic ve-muted">加载中...</span></div>`.appendTo(eleModalInner);
		return doClose;
	}
	// endregion
}
