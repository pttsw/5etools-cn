...
class SearchWidget {
		...
		if (resultCount) {
			...
			if (resultCount > UiUtil.SEARCH_RESULTS_CAP) {
				...
				if (I18nUtil.LANGUAGES_INDEX === "zh_CN") {
					this._wrpResults.vee.appends(`<div class="ve-ui-search__row ve-ui-search__row--readonly">...${diff}条结果被隐藏，请调整搜索条件。</div>`);
				} else {
	...
	static _showLoadingModal () {
		...
		veT`<div class="ve-flex-vh-center ve-w-100 ve-h-100"><span class="ve-dnd-font ve-italic ve-muted">加载中...</span></div>`.vee.appendTo(eleModalInner);
		return doClose;
	}
	// endregion
}
...
class InputUiUtil {
	...
	static _getBtnCancel ({comp = null, opts, doClose}) {
		return veT`<button class="ve-btn ve-btn-default">取消</button>`
			.vee.onn("click", evt => {
				evt.stopPropagation();
				doClose(false);
			});
	...
	static _getBtnSkip ({comp = null, opts, doClose}) {
		return !opts.isSkippable ? null : veT`<button class="ve-btn ve-btn-default ve-ml-3">跳过</button>`
			.vee.onn("click", evt => {
				evt.stopPropagation();
	...
	static async pGetUserEnum (opts) {
		...
		if (opts.isAllowNull) veT`<option value="-1"></option>`.vee.txt(opts.fnDisplay ? opts.fnDisplay(null, -1) : "(无)").vee.appendTo(selEnum);
...
class DragReorderUiUtil {
	...
	static getDragPadOpts (fnGetRow, opts) {
		...
		return veT`<div class="ve-mr-2 ve-ui-drag__patch" title="拖拽排序">
