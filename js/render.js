...
globalThis.Renderer = function () {
	...
	this._getPtExpandCollapse = function () {
		return `<span class="ve-rd__h-toggle ve-ml-2 ve-clickable ve-no-select no-print ve-lst-is-exporting-image__hidden" data-rd-h-toggle-button="true" title="折叠/展开（按住CTRL切换所有）">[\u2013]</span>`;
	};

	this._getPtExpandCollapseSpecial = function () {
		return `<span class="ve-rd__h-toggle ve-ml-2 ve-clickable ve-no-select no-print ve-lst-is-exporting-image__hidden" data-rd-h-special-toggle-button="true" title="折叠/展开（按住CTRL切换所有）">[\u2013]</span>`;
	};
	...
	this._renderActions = function (entry, textStack, meta, options) {
		...
		const headerSpan = `<span class="ve-rd__h ve-rd__h--3" data-title-index="${this._headerIndex++}" ${this._getEnumeratedTitleRel(entry.name)}><span class="entry-title-inner">${entry.name}。</span><span class="entry-title-inner">${entry.ENG_name}.</span></span> `;
