...
globalThis.Renderer = class {
	...
	_renderEntriesSubtypes_renderPreReqText (entry, textStack, meta) {
		...
		if (entry.type === "optfeature") {
			textStack[0] += `<span class="ve-rd__prerequisite">先决条件: `;
	...
	_getPtExpandCollapse () {
		return `<span class="ve-rd__h-toggle ve-ml-2 ve-clickable ve-no-select no-print ve-lst-is-exporting-image__hidden" data-rd-h-toggle-button="true" title="折叠/展开（按住SHIFT切换所有）">[\u2212]</span>`;
	}

	_getPtExpandCollapseSpecial () {
		return `<span class="ve-rd__h-toggle ve-ml-2 ve-clickable ve-no-select no-print ve-lst-is-exporting-image__hidden" data-rd-h-special-toggle-button="true" title="折叠/展开（按住SHIFT切换所有）">[\u2212]</span>`;
	}
	...
	_renderAbilityDc (entry, textStack, meta, options) {
		...
		if (options.styleHint === "classic") textStack[0] += ` 豁免DC</b> = 8 + 你的熟练加值 + 你的${Parser.attrChooseToFull(entry.attributes)}</div>`;
		else textStack[0] += ` 豁免DC</b> = 8 + ${Parser.attrChooseToFull(entry.attributes)} + 熟练加值</div>`;
	}
	...
	_renderActions (entry, textStack, meta, options) {
		...
		const headerSpan = `<span class="ve-rd__h ve-rd__h--3" ${this._getRenderedTitleIndexAttribute()} ${this._getEnumeratedTitleRel(entry.name)}><span class="entry-title-inner">${entry.name}。</span><span class="entry-title-inner">${entry.ENG_name}.</span></span> `;
...
Renderer.attackTagToFull = function (tagStr, {isRoll = false} = {}) {
	function renderTag (tags) {
		const ptType = tags.includes("m") ? "近战" : tags.includes("r") ? "远程" : tags.includes("g") ? "魔法" : tags.includes("a") ? "范围" : "";
		const ptMethod = tags.includes("w") ? "武器" : tags.includes("s") ? "法术" : tags.includes("p") ? "Power " : "";
...
Renderer.getAbilityData._doRenderOuter = function (abObj) {
	...
	function handleAbilitiesChooseWeighted () {
		...
		const areReduce = isAny && isAllEqual && w.weights.length > 1 && w.weights[0] < 0
			? (() => {
				...
				return [`${cntProcessed ? "选择 " : ""}${Parser.numberToText(w.weights.length)} 个不同的 ${UiUtil.intToBonus(weightsReduce[0], {isPretty: true})}`];
			})()
		...
		const startText = isAny
			? `选择 `
			: `从 ${w.from.map(it => Parser.attAbvToFull(it)).joinConjunct(", ", " 和 ")} 中选择 `;
	...
	function handleAbilitiesChooseFrom () {
		...
		if (ptsLong.length) toConvertToText.push(`选择 ${ptsLong.join(" ")}`);
...
Renderer.utils = class {
	...
	static prerequisite = class {
		...
		static _getEntry_feature ({v, isListMode, keyOptions, styleHint}) {
			...
			const ptNames = v.joinConjunct(", ", " 或 ");
		...
		static _getEntry_spellcastingPrepared ({v, isListMode}) {
			return isListMode ? "施法能力" : "来自职业或者准备法术的施法能力";
		}
		...
		static _getEntry_spellcastingFocus ({v, isListMode, styleHint}) {
			if (isListMode) {
				if (v === true) return `法器`;
	...
	static getTagEntry (tag, text) {
		switch (tag) {
			...
			case "@recharge": {
				...
				if ((!fauxEntry.displayText && (rollText || "").includes("summonSpellLevel")) || (fauxEntry.displayText && fauxEntry.displayText.includes("summonSpellLevel"))) fauxEntry.displayText = (fauxEntry.displayText || rollText || "").replace(/summonSpellLevel/g, "法术环阶");

				if ((!fauxEntry.displayText && (rollText || "").includes("summonClassLevel")) || (fauxEntry.displayText && fauxEntry.displayText.includes("summonClassLevel"))) fauxEntry.displayText = (fauxEntry.displayText || rollText || "").replace(/summonClassLevel/g, "你的职业等级");
...
Renderer.class = class {
	...
	static getHtmlPtHitPoints (cls, {renderer = null, styleHint = null}) {
		...
		<div><strong>生命值 每额外${cls.name}等级:</strong> ${Renderer.class.getHitPointsAtHigherLevels(cls.name, cls.hd, {styleHint})}</div>`;
	}

	static getHtmlPtSavingThrows (cls) {
		if (!cls.proficiency) return "";

		return `<div><b>豁免熟练:</b> <span>${cls.proficiency.map(p => Parser.attAbvToFull(p)).join(", ")}</span></div>`;
	}
...
class _RenderCompactBestiaryImplBase {
	...
	_getCommonHtmlParts_crSpellLevel ({mon, opts, isShowCrScaler, isShowSpellLevelScaler, isShowClassLevelScaler}) {
		...
		if (isShowCrScaler) {
			return `<td colspan="2">
				...
				<button title="基于CR调整数据（实验功能）" class="mon__btn-scale-cr ve-btn ve-btn-xs ve-btn-default no-print">
	...
	_getCommonHtmlParts_variants ({mon, renderer}) {
		...
		${mon.summonedBySpell ? `<div><b>通过...召唤:</b> ${renderer.render(`{@spell ${mon.summonedBySpell}}`)}<div>` : ""}
		</td></tr>`;
	}
}
...
`,
	...
	static _getChallengeRatingPart_one ({mon, isPlainText = false} = {}) {
		...
		const ptParens = [
			ptsXp.length ? `XP ${ptsXp.joinConjunct(", ", ", 或 ", true)}` : "",
...
Renderer.legendaryGroup = class {
	...
	static getSummaryEntry (legGroup) {
		...
		return {
			type: "section",
			entries: [
				legGroup.lairActions ? {name: "巢穴动作", type: "entries", entries: legGroup.lairActions} : null,
				legGroup.regionalEffects ? {name: "区域效应", type: "entries", entries: legGroup.regionalEffects} : null,
				legGroup.mythicEncounter ? {name: "作为一个神话遭遇", type: "entries", entries: legGroup.mythicEncounter} : null,
			].filter(Boolean),
		};
	}
};
...
Renderer.item = class {
	...
	static _createSpecificVariants_isEditionMatch ({curBaseItem, curGenericVariant, styleHint}) {
		...
		if (globalThis.IS_VTT && styleHint === "classic") {
			if (curBaseItem.edition === "one" || curBaseItem.edition === "一") return false;
			if (curBaseItem.edition == null) return true;
			if (curBaseItem.edition === "classic" || curBaseItem.edition === "经典") return curGenericVariant.edition !== "one";
		}
	...
	static enhanceItem (item, {styleHint = null} = {}) {
		...
		if (itemTypeAbv === Parser.ITM_TYP_ABV__GENERIC_VARIANT) item._category = "通用变体";
...
Renderer.vehicle = class {
	...
	static ship = class {
		...
		static getVehicleShipRenderableEntriesMeta (ent) {
			...
			return {
				entrySizeDimensions: `{@i ${Parser.sizeAbvToFull(ent.size)}载具${ptDimensionsTerrain ? ` (${ptDimensionsTerrain})` : ""}}`,
				entryCreatureCapacity: ent.capCrew != null || ent.capPassenger != null
					? `{@b 生物容量} ${Renderer.vehicle.getShipCreatureCapacity(ent)}`
					: null,
				entryCargoCapacity: ent.capCargo != null
