...
class SearchPage {
	...
	static _render () {
		const iptSearch = ee`<input class="form-control pg-search__ipt" placeholder="全局搜索.." title="免责声明：不一定真的达到全域搜索，请谨慎使用。">`
			.onn("keydown", evt => {
		...
		const btnHelp = ee`<button class="ve-btn ve-btn-default mr-2 mobile-sm__hidden" title="帮助"><span class="glyphicon glyphicon-info-sign"></span></button>`
		...
		SearchPage._wrpResults = ee`<div class="ve-flex-col w-100">${this._getWrpResult_message("加载中...")}</div>`;
		...
		ee(SearchPage._wrp)`<div class="ve-flex-col w-100 pg-search__wrp">
			<div class="ve-flex-v-center mb-2 mobile-lg__ve-flex-col">
				...
				<div class="ve-flex-v-center mobile-sm__ve-flex-col mobile-lg__ve-flex-ai-start mobile-lg__w-100">
					${btnHelp}
					<div class="mr-2 ml-1 mobile-sm__ml-0 mobile-sm__mb-2 italic">包括</div>
	...
	static async _pDoSearch () {
		...
		SearchPage._rowMetas = results.map(result => {
			...
			if (isHoverable) {
				...
				const btnTogglePreview = ee`<button class="ve-btn ve-btn-default ve-btn-xs h-100" title="切换预览"></button>`
					.onn("click", () => {
