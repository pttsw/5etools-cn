...
class SearchPage {
	...
	static _render () {
		const iptSearch = ee`<input class="ve-form-control pg-search__ipt" placeholder="全局搜索.." title="免责声明：不一定真的达到全域搜索，请谨慎使用。">`
			.onn("keydown", evt => {
		...
		const btnHelp = ee`<button class="ve-btn ve-btn-default ve-mr-2 ve-mobile-sm__hidden" title="帮助"><span class="glyphicon glyphicon-info-sign"></span></button>`
		...
		const btnCollapseAll = ee`<button class="ve-btn ve-btn-default" title="折叠所有结果"><span class="glyphicon glyphicon-minus"></span></button>`
			.onn("click", () => handleMassExpandCollapse(false));

		const btnExpandAll = ee`<button class="ve-btn ve-btn-default" title="展开所有结果"><span class="glyphicon glyphicon-plus"></span></button>`
			.onn("click", () => handleMassExpandCollapse(true));

		SearchPage._wrpResults = ee`<div class="ve-flex-col ve-w-100">${this._getWrpResult_message("加载中...")}</div>`;
	...
	static async _pDoSearch () {
		...
		if (!params[this._PARAM_QUERY]) {
			SearchPage._wrpResults.empty().appends(this._getWrpResult_message("输入搜索内容以查看结果"));
			return;
		}
		...
		if (!results.length) {
			SearchPage._wrpResults.appends(this._getWrpResult_message("未搜索到结果。"));
			return;
		}
		...
		SearchPage._rowMetas = results.map(result => {
			...
			if (isHoverable) {
				...
				const btnTogglePreview = ee`<button class="ve-btn ve-btn-default ve-btn-xs ve-h-100" title="切换预览"></button>`
					.onn("click", () => {
						...
						DataLoader.pCacheAndGet(
							...
							switch (category) {
								...
								case Parser.CAT_ID_ADVENTURE: {
									...
									isImagePopulated = true;
									dispImage.html(`<img src="${Renderer.adventureBook.getCoverUrl(ent[prop])}" class="ve-w-100 ve-h-100" alt="封面图: ${(ent[prop].name || "").qq()}" loading="lazy">`);
								}
							}
