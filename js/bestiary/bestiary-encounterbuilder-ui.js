...
export class EncounterBuilderUiBestiary extends EncounterBuilderUi {
	...
	_render_groupAndDifficulty ({rdState, parentGroupAndDifficulty}) {
		...
		const btnSaveToUrl = ee`<button class="ve-btn ve-btn-default ve-btn-xs mr-2" data-i18n="common.button.save_to_file"></button>`
			.onn("click", () => this._sublistManager.pHandleClick_download({isUrl: true, eleCopyEffect: btnSaveToUrl}));
		const btnSaveToFile = ee`<button class="ve-btn ve-btn-default ve-btn-xs" data-i18n="common.button.save_to_file"></button>`
			.onn("click", () => this._sublistManager.pHandleClick_download());
		const btnLoadFromFile = ee`<button class="ve-btn ve-btn-default ve-btn-xs" data-i18n="common.button.load_from_file"></button>`
			.onn("click", evt => this._sublistManager.pHandleClick_upload({isAdditive: evt.shiftKey}));
		const btnCopyAsText = ee`<button class="ve-btn ve-btn-default ve-btn-xs mr-2" data-i18n-title="page.enconterbuilder.shift_for_multi_line_format" data-i18n="common.button.copy_as_text"></button>`.onn("click", (evt) => this._handleClickCopyAsText(evt));
		const btnReset = ee`<button class="ve-btn ve-btn-danger ve-btn-xs" data-i18n-title="page.enconterbuilder.shift_to_reset_players" data-i18n="common.button.reset"></button>`
			.onn("click", (evt) => this._sublistManager.pHandleClick_new(evt));

		const btnBackToStatblocks = ee`<button class="ve-btn ve-btn-success ve-btn-xs" data-i18n="page.enconterbuilder.back_to_stat_blocks"></button>`.onn("click", (evt) => this._handleClickBackToStatblocks(evt));
