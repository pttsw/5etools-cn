...
export class EncounterBuilderUiBestiary extends EncounterBuilderUi {
	...
	_doUpdateDispSummary () {
		...
		this._dispSummary.vee.html(`${monCount}个生物; ${this._getActiveRulesComp().getDisplaySummary()}`);
	}
	...
	getSublistButtonsMeta (sublistItem) {
		const btnAdd = veT`<button title="新增 (按住SHIFT加5个)" class="ve-btn ve-btn-success ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-plus"></span></button>`
			.vee.onn("click", evt => this._handleClick({evt, entity: sublistItem.data.entity, mode: "add"}));

		const btnSub = veT`<button title="减少 (按住SHIFT减5个)" class="ve-btn ve-btn-danger ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-minus"></span></button>`
			.vee.onn("click", evt => this._handleClick({evt, entity: sublistItem.data.entity, mode: "subtract"}));

		const btnRandomize = veT`<button title="随机怪物" class="ve-btn ve-btn-default ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-random"></span></button>`
			.vee.onn("click", evt => this._pHandleShuffleClick({evt, sublistItem}));

		const btnLock = veT`<button title="锁定怪物防止被随机/调整" class="ve-btn ve-btn-default ve-btn-xs best-ecgen__btn-list"><span class="glyphicon glyphicon-lock"></span></button>`
