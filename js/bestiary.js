...
class BestiarySublistManager extends SublistManager {
	...
	async pGetSublistItem (mon, hash, {count = 1, customHashId = null, initialData} = {}) {
		...
		const hovStatblock = ee`<span class="ve-col-1-4 ve-help ve-help--hover best-ecgen__visible" data-i18n='common.tabs.stat_block'></span>`
		...
		const hovImage = ee`<span class="ve-col-1-2 best-ecgen__visible ve-help ve-help--hover">图片</span>`;
...
class BestiaryPage extends ListPageMultiSource {
			...
			children: [
				e_({
					...
					children: [
						...
						e_({tag: "span", clazz: `ve-col-1-7 ve-px-1 ve-text-center`, text: translator}),
						e_({
							tag: "span",
	...
	_renderStatblock_doBuildStatsTab (
		...
		const btnScaleCr = !ScaleCreature.isCrInScaleRange(mon) ? null : ee`<button id="btn-scale-cr" title="基于CR调整数据（实验功能）" class="mon__btn-scale-cr ve-btn ve-btn-xs ve-btn-default ve-popwindow__hidden no-print ve-lst-is-exporting-image__hidden"><span class="glyphicon glyphicon-signal"></span></button>`
			.onn("click", (evt) => {
				evt.stopPropagation();
