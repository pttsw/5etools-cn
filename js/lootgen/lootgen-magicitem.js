...
export class LootGenMagicItem extends BaseComponent {
	...
	_getBtnReroll () {
		...
		const title = this._itemsAltChoose
			...
			: null;
		return ee`<span class="roller render-roller" ${title ? `title="${title}"` : ""}>[${I18nUtil.get("common.button.reroll")}]</span>`
...
class LootGenMagicItemSpellScroll extends LootGenMagicItem {
	...
	_getRender () {
		...
		const btnRerollSpell = ee`<span class="roller render-roller mr-2">[${I18nUtil.get("common.button.reroll")}]</span>`
...
class LootGenMagicItemSubItems extends LootGenMagicItem {
	...
	_getRender () {
		...
		const btnRerollSubItem = ee`<span class="roller render-roller mr-2">[${I18nUtil.get("common.button.reroll")}]</span>`
			.onn("mousedown", evt => evt.preventDefault())
			.onn("click", () => {
...
export class LootGenMagicItemTable extends LootGenMagicItem {
	...
	_getRender () {
		...
		const btnRerollSub = ee`<span class="roller render-roller ve-small ve-self-flex-end">[${I18nUtil.get("common.button.reroll")}]</span>`
			.onn("mousedown", evt => evt.preventDefault())
			.onn("click", async () => {
				const {subRowRoll, subRow, subItem} = await LootGenMagicItemTable.pGetSubRollMeta({
