export class StatGenUiRenderLevelOneEntityBase {
	...
	render () {
		...
		const btnFilterForEntity = veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-br-0 ve-pr-2" title="Filter for ${this._title}"><span class="glyphicon glyphicon-filter"></span> 筛选</button>`
			.vee.onn("click", async () => {
		...
		const stgAbilityScoreSet = veT`<div class="ve-flex-v-center ve-mb-2">
			<div class="ve-mr-2 ve-no-wrap">属性值提升</div>
		...
		const stgSel = veT`<div class="ve-flex-col ve-mt-3">
			<div class="ve-mb-1">选择一个${this._title}</div>
	...
	_render_pointBuy (wrp) {
		...
		if (Parser.ABIL_ABVS.some(it => fromEntity[it])) {
			...
			ptBase = veT`<div class="ve-flex-col ve-mr-3">
				<div class="ve-my-1 ve-statgen-pb__header ve-flex-vh-center">静态</div>
				${wrpsEntity}
			</div>`;
		}
