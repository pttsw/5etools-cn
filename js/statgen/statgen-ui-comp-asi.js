...
export class StatGenUiCompAsi extends BaseComponent {
		...
		const hk = () => {
			...
			for (; ix < this._parent.state[propCnt]; ++ix) {
				...
				if (!this._metasAsi[namespace][ix_]) {
					...
					const btnAsi = namespace !== "ability" ? null : ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-w-50p">属性</button>`
						.onn("click", () => {
							this._parent.state[propMode] = "asi";
							this._doPulseThrottled();
						});

					const btnFeat = namespace !== "ability" ? ee`<div class="ve-w-100p ve-text-center">${I18nUtil.get("common.feat")}</div>` : ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-w-50p">专长</button>`
						.onn("click", () => {
	...
	_render_getMetaFeat ({featStatic = null, propIxFeat = null, propIxFeatAbility, propFeatAbilityChooseFrom, category = null}) {
		...
		const hkIxFeat = (prop) => {
			...
			dispFeat.html(feat ? Renderer.get().render(`{@feat ${VetoolsConfig.get("styleSwitcher", "style") === SITE_STYLE__CLASSIC ? feat.name.toLowerCase() : feat.name}|${feat.source}}`) : `(选择一项专长)`);
	...
	_render_getStageCntAsi () {
		if (!this._parent.isCharacterMode) {
			...
			return ee`<label class="ve-w-100 ve-flex-v-center ve-mb-2"><div class="ve-mr-2 ve-no-shrink">属性值提升次数:</div>${wrpCountAsi}</label>`;
		}

		const out = ee`<div class="ve-w-100 ve-flex-v-center ve-mb-2 ve-italic ve-muted">没有可以提升的属性值。</div>`;
