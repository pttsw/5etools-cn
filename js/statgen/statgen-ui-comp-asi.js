...
export class StatGenUiCompAsi extends BaseComponent {
		...
		const hk = () => {
			...
			for (; ix < this._parent.state[propCnt]; ++ix) {
				...
				if (!this._metasAsi[namespace][ix_]) {
					...
					const btnAsi = namespace !== "ability" ? null : veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-w-50p">属性</button>`
						.vee.onn("click", () => {
							this._parent.state[propMode] = "asi";
							this._doPulseThrottled();
						});

					const btnFeat = namespace !== "ability" ? veT`<div class="ve-w-100p ve-text-center">${I18nUtil.get("common.feat")}</div>` : veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-w-50p">专长</button>`
						.vee.onn("click", () => {
			...
			const hkIxSel = (prop) => {
				...
				uidsStatic.map((uid, ix) => {
					...
					const row = veT`<div class="ve-flex-v-end ve-py-3 ve-px-1 ve-statgen-asi__row">
						<div class="ve-btn-group"><div class="ve-w-100p ve-text-center">${I18nUtil.get("common.feat")}</div></div>
						<div class="ve-vr-4"></div>
						${stgFeat}
	...
	_render_getMetaFeat ({featStatic = null, propIxFeat = null, propIxFeatAbility, propFeatAbilityChooseFrom, category = null}) {
		...
		const hkIxFeat = (prop) => {
			...
			dispFeat.vee.html(feat ? Renderer.get().render(`{@feat ${VetoolsConfig.get("styleSwitcher", "style") === SITE_STYLE__CLASSIC ? feat.name.toLowerCase() : feat.name}|${feat.source}}`) : `(选择一项专长)`);
	...
	render (wrpAsi) {
		...
		const getStgEntity = ({title, wrpRows, propEntity, propIxEntity}) => {
			const stg = veT`<div class="ve-flex-col">
				...
				<h4 class="ve-my-2 ve-bold">${title}专长</h4>
				${wrpRows}
			</div>`;
		...
		veT(wrpAsi)`
			<h4 class="ve-my-2 ve-bold">属性值提升</h4>
			...
			<hr class="ve-hr-3 ve-hr--dotted">
			<h4 class="ve-my-2 ve-bold">${I18nUtil.get("page.statgen.additional_feats")}</h4>
			<label class="ve-w-100 ve-flex-v-center ve-mb-2">
				<div class="ve-mr-2 ve-no-shrink">${I18nUtil.get("page.statgen.number_of_additional_feats")}:</div>${wrpCountFeatsCustom}
			</label>
			${wrpRowsCustom}
		`;
	}
	...
	_render_getStageCntAsi () {
		if (!this._parent.isCharacterMode) {
			...
			return veT`<label class="ve-w-100 ve-flex-v-center ve-mb-2"><div class="ve-mr-2 ve-no-shrink">属性值提升次数:</div>${wrpCountAsi}</label>`;
		}

		const out = veT`<div class="ve-w-100 ve-flex-v-center ve-mb-2 ve-italic ve-muted">没有可以提升的属性值。</div>`;
