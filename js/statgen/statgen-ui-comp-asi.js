...
export class StatGenUiCompAsi extends BaseComponent {
		...
		const hk = () => {
			...
			for (; ix < this._parent.state[propCnt]; ++ix) {
				...
				if (!this._metasAsi[namespace][ix_]) {
					...
					const btnAsi = namespace !== "ability" ? null : ee`<button class="ve-btn ve-btn-xs ve-btn-default w-50p">属性</button>`
						.onn("click", () => {
							this._parent.state[propMode] = "asi";
							this._doPulseThrottled();
						});

					const btnFeat = namespace !== "ability" ? ee`<div class="w-100p ve-text-center">${I18nUtil.get("common.feat")}</div>` : ee`<button class="ve-btn ve-btn-xs ve-btn-default w-50p">专长</button>`
						.onn("click", () => {
	...
	_render_getStageCntAsi () {
		if (!this._parent.isCharacterMode) {
			...
			return ee`<label class="w-100 ve-flex-v-center mb-2"><div class="mr-2 no-shrink">属性值提升次数:</div>${wrpCountAsi}</label>`;
		}

		const out = ee`<div class="w-100 ve-flex-v-center mb-2 italic ve-muted">没有可以提升的属性值。</div>`;
