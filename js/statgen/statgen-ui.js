...
export class StatGenUi extends BaseComponent {
	...
	_render_getStgRolledHeader () {
		...
		const btnRoll = ee`<button class="ve-btn ve-btn-primary ve-bold">掷骰</button>`
			.onn("click", async () => {
				try {
		...
		const btnRandom = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mt-2">随机分配</button>`
			.hideVe()
			.onn("click", () => {
		...
		const hkRolled = () => {
			...
			wrpTotal.txt(`总计: ${this._state.rolled_rolls.map(roll => roll.total).sum()}`);
		};
		...
		return ee`<div class="ve-flex-col ve-mb-3 ve-mr-auto">
			<div class="ve-flex ve-mb-2">
				<div class="ve-flex-col ve-flex-h-center ve-mr-3">
					...
					${this._isCharacterMode ? null : ee`<label class="ve-flex-v-center ve-mt-2"><div class="ve-mr-2 ve-no-shrink ve-w-100p">掷骰次数:</div>${iptRollCount}</label>`}
				</div>
				${btnRoll}
			</div>
	...
	_render_getStgArrayHeader () {
		const btnRandom = ee`<button class="ve-btn ve-btn-xs ve-btn-default">随机分配</button>`
			.onn("click", () => {
				const abs = [...Parser.ABIL_ABVS].shuffle();
				abs.forEach((ab, i) => {
	...
	_render_getStgPbHeader () {
		...
		const btnRandom = ee`<button class="ve-btn ve-btn-default">随机</button>`
			.onn("click", () => {
				this._doReset();
	...
	_render_getStgPbCustom () {
		const btnAddLower = ee`<button class="ve-btn ve-btn-default ve-btn-xs">添加更低的属性值</button>`
			.onn("click", () => {
		...
		const btnAddHigher = ee`<button class="ve-btn ve-btn-default ve-btn-xs">添加更高的属性值</button>`
			.onn("click", () => {
		...
		const btnResetRules = ee`<button class="ve-btn ve-btn-danger ve-btn-xs ve-mr-2">重置</button>`
			.onn("click", () => {
	...
	_render_isLevelOne (wrpTab) {
		...
		const btnResetRolledOrArrayOrManual = ee`<button class="ve-btn ve-btn-default ve-btn-xxs ve-relative ve-statgen-shared__btn-reset" title="重置"><span class="glyphicon glyphicon-refresh"></span></button>`
			.onn("click", () => this._doReset());
