...
export class StatGenUi extends BaseComponent {
	...
	_render_getStgRolledHeader () {
		...
		const btnRoll = ee`<button class="ve-btn ve-btn-primary bold">掷骰</button>`
			.onn("click", async () => {
				try {
		...
		const btnRandom = ee`<button class="ve-btn ve-btn-xs ve-btn-default mt-2">随机分配</button>`
			.hideVe()
			.onn("click", () => {
		...
		return ee`<div class="ve-flex-col mb-3 mr-auto">
			<div class="ve-flex mb-2">
				<div class="ve-flex-col ve-flex-h-center mr-3">
					<label class="ve-flex-v-center"><div class="mr-2 no-shrink w-100p">公式:</div>${this._rollIptFormula}</label>

					${this._isCharacterMode ? null : ee`<label class="ve-flex-v-center mt-2"><div class="mr-2 no-shrink w-100p">掷骰次数:</div>${iptRollCount}</label>`}
				</div>
				${btnRoll}
			</div>
	...
	_render_getStgPbCustom () {
		...
		const btnResetRules = ee`<button class="ve-btn ve-btn-danger ve-btn-xs mr-2">重置</button>`
			.onn("click", () => {
	...
	_render_isLevelOne (wrpTab) {
		...
		const btnResetRolledOrArrayOrManual = ee`<button class="ve-btn ve-btn-default ve-btn-xxs relative statgen-shared__btn-reset" title="重置"><span class="glyphicon glyphicon-refresh"></span></button>`
			.onn("click", () => this._doReset());
			...
			<div class="ve-flex mobile-lg__ve-flex-col w-100 px-3">
				<div class="ve-flex-col">
					...
					<div class="ve-flex">
						...
						<div class="ve-flex-col mr-3">
							<div class="my-1 statgen-pb__header"></div>
							<div class="my-1 statgen-pb__header ve-flex-vh-center help ve-muted" title="此处输入额外/自定义的加值">用户</div>
							${wrpsUser}
						</div>
