...
export class StatGenUi extends BaseComponent {
	...
	render (parent) {
		...
		const iptTabMetas = this._isLevelUp
			...
			: [
				...
				new TabUiUtil.TabMeta({name: "标准数组", icon: this._isFvttMode ? `fas fa-fw fa-signal` : `far fa-fw fa-signal-bars`, hasBorder: true, isNoPadding: this._isFvttMode}),
				...
				new TabUiUtil.TabMeta({name: "手动", icon: this._isFvttMode ? `fas fa-fw fa-screwdriver-wrench` : `far fa-fw fa-screwdriver-wrench`, hasBorder: true, isNoPadding: this._isFvttMode}),
				...this._tabMetasAdditional || [],
			].filter(Boolean);
	...
	_render_getStgRolledHeader () {
		...
		const btnRoll = veT`<button class="ve-btn ve-btn-primary ve-bold">掷骰</button>`
			.vee.onn("click", async () => {
				try {
		...
		const btnRandom = veT`<button class="ve-btn ve-btn-xs ve-btn-default ve-mt-2">随机分配</button>`
			.vee.hide()
			.vee.onn("click", () => {
		...
		const hkRolled = () => {
			...
			wrpTotal.vee.txt(`总计: ${this._state.rolled_rolls.map(roll => roll.total).sum()}`);
		};
		...
		return veT`<div class="ve-flex-col ve-mb-3 ve-mr-auto">
			<div class="ve-flex ve-mb-2">
				<div class="ve-flex-col ve-flex-h-center ve-mr-3">
					<label class="ve-flex-v-center"><div class="ve-mr-2 ve-no-shrink ve-w-100p">公式:</div>${this._rollIptFormula}</label>

					${this._isCharacterMode ? null : veT`<label class="ve-flex-v-center ve-mt-2"><div class="ve-mr-2 ve-no-shrink ve-w-100p">掷骰次数:</div>${iptRollCount}</label>`}
				</div>
				${btnRoll}
			</div>
	...
	_render_getStgArrayHeader () {
		const btnRandom = veT`<button class="ve-btn ve-btn-xs ve-btn-default">随机分配</button>`
			.vee.onn("click", () => {
				const abs = [...Parser.ABIL_ABVS].shuffle();
				abs.forEach((ab, i) => {
		...
		return veT`<div class="ve-flex-col ve-mb-3 ve-mr-auto">
			<div class="ve-mb-2">将这些数值任意分配到你的属性上:</div>
	...
	_render_getStgPbHeader () {
		...
		const btnReset = veT`<button class="ve-btn ve-btn-default">重置</button>`
			.vee.onn("click", () => this._doReset());

		const btnRandom = veT`<button class="ve-btn ve-btn-default">随机</button>`
			.vee.onn("click", () => {
				this._doReset();
		...
		return veT`<div class="ve-flex ve-mobile-sm__flex-col ve-mb-2">
			<div class="ve-flex-v-center">
				...
				<label class="ve-flex-col ve-mr-2">
					<div class="ve-mb-1 ve-text-center">总点数</div>
					${iptBudget}
				</label>

				<label class="ve-flex-col ve-mr-2">
					<div class="ve-mb-1 ve-text-center">剩余点</div>
					${iptRemaining}
				</label>
			</div>
	...
	_render_getStgPbCustom () {
		const btnAddLower = veT`<button class="ve-btn ve-btn-default ve-btn-xs">添加更低的属性值</button>`
			.vee.onn("click", () => {
		...
		const btnAddHigher = veT`<button class="ve-btn ve-btn-default ve-btn-xs">添加更高的属性值</button>`
			.vee.onn("click", () => {
		...
		const btnResetRules = veT`<button class="ve-btn ve-btn-danger ve-btn-xs ve-mr-2">重置</button>`
			.vee.onn("click", () => {
				...
				<div class="ve-flex ve-mobile-sm__flex-col">
					<div class="ve-flex-col ve-mr-3 ve-mobile-sm__mr-0">
						<div class="ve-flex-v-center ve-mb-1">
							<div class="ve-statgen-pb__col-cost ve-flex-vh-center ve-bold">属性值</div>
							<div class="ve-statgen-pb__col-cost ve-flex-vh-center ve-bold">调整值</div>
							<div class="ve-statgen-pb__col-cost ve-flex-vh-center ve-bold">花费点数</div>
	...
	_render_isLevelOne (wrpTab) {
		...
		const btnResetRolledOrArrayOrManual = veT`<button class="ve-btn ve-btn-default ve-btn-xxs ve-relative ve-statgen-shared__btn-reset" title="重置"><span class="glyphicon glyphicon-refresh"></span></button>`
			.vee.onn("click", () => this._doReset());
				...
				<div class="ve-flex-col">
					...
					<div class="ve-flex">
						<div class="ve-flex-col ve-mr-3">
							...
							${Parser.ABIL_ABVS.map(it => `<div class="ve-my-1 ve-bold ve-statgen-pb__cell ve-flex-v-center ve-flex-h-right" title="${Parser.attAbvToFull(it)}">${Parser.attAbvToFull(it)}</div>`)}
						</div>

						<div class="ve-flex-col ve-mr-3">
							<div class="ve-my-1 ve-statgen-pb__header"></div>
							<div class="ve-my-1 ve-bold ve-statgen-pb__header ve-flex-vh-center">基础</div>
							${wrpsBase}
						</div>
						...
						<div class="ve-flex-col ve-mr-3">
							<div class="ve-my-1 ve-statgen-pb__header"></div>
							<div class="ve-my-1 ve-statgen-pb__header ve-flex-vh-center ve-help ve-muted" title="此处输入额外/自定义的加值">用户</div>
							${wrpsUser}
						</div>

						<div class="ve-flex-col ve-mr-3">
							<div class="ve-my-1 ve-statgen-pb__header"></div>
							<div class="ve-my-1 ve-statgen-pb__header ve-flex-vh-center">总计</div>
							${metasTotalAndMod.map(it => it.wrpIptTotal)}
						</div>

						<div class="ve-flex-col ve-mr-3">
							<div class="ve-my-1 ve-statgen-pb__header"></div>
							<div class="ve-my-1 ve-statgen-pb__header ve-flex-vh-center" title="Modifier">调整值</div>
