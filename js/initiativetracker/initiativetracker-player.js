...
export class InitiativeTrackerPlayerMessageHandlerV1 {
	...
	_handleMessage_state ({payload}) {
		...
		this._eleHead.vee.appends(`
				<div class="ve-w-100 ve-split-v-center ve-min-w-100p ${this._isCompact ? "ve-text-center" : ""}">${I18nUtil.get("page.dmscreen.creature_status")}</div>
				<div class="ve-min-w-100p ${this._isCompact ? "ve-text-center" : ""}">生命值</div>
	...
	_getRenderedStatsCellInner_image ({cell}) {
		...
		const ele = veT`<img src="${tokenUrl}" class="ve-w-24p ve-w-24p" alt="Token图">`
