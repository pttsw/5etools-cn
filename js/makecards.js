...
class MakeCards extends BaseComponent {
	...
	_render_configSection () {
		...
		const btnResetDefaults = ee`<button class="ve-btn ve-btn-default ve-btn-xs">重置</button>`
			.onn("click", () => {
	...
	_render_cardList () {
		...
		const btnAdd = ee`<button class="ve-btn ve-btn-primary ve-mr-2"><span class="glyphicon glyphicon-plus"></span> 添加</button>`
			.onn("click", evt => ContextUtil.pOpenMenu(evt, menuSearch));
		const btnReset = ee`<button class="ve-btn ve-btn-danger ve-mr-2"><span class="glyphicon glyphicon-trash"></span> 重置</button>`
			.onn("click", async () => {
	...
	static _pGetUserIcon (initialVal) {
		return new Promise(resolve => {
			...
			const btnOk = ee`<button class="ve-btn ve-btn-default">确认</button>`
