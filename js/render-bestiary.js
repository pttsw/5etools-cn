...
class _RenderBestiaryImplClassic extends _RenderBestiaryImplBase {
	...
	_getRenderedCreature ({mon, opts, renderer}) {
		...
		<tr>${opts.selSummonSpellLevel ? ee`<td colspan="6"><strong class="ve-mr-2">法术环阶</strong> ${opts.selSummonSpellLevel}</td>` : ""}</tr>
		<tr>${opts.selSummonClassLevel ? ee`<td colspan="6"><strong class="ve-mr-2">${opts.classLevelScalerClass ? "职业等级" : "等级"}</strong> ${opts.selSummonClassLevel}</td>` : ""}</tr>
