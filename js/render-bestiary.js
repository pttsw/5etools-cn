...
class _RenderBestiaryImplBase {
	...
	_getCommonHtmlParts_speed ({mon, isInlinedToken}) {
		return `<tr><td colspan="6"><div ${this._style !== "classic" && isInlinedToken ? `class="ve-stats__wrp-avoid-token"` : ""}><strong>速度</strong> ${Parser.getSpeedString(mon, {styleHint: this._style})}</div></td></tr>`;
	}
	...
	_getCommonHtmlParts_tools ({mon, renderer}) {
		return mon.tool ? `<tr><td colspan="6"><strong>工具</strong> ${Renderer.monster.getToolsString(renderer, mon, {styleHint: this._style})}</td></tr>` : "";
	}
	...
	_getCommonHtmlParts_senses ({mon}) {
		...
		return pt ? `<tr><td colspan="6"><strong>感知</strong> ${pt}</td></tr>` : "";
	}
	...
	_getCommonHtmlParts_legendaryActions ({mon, entsLegendaryAction}) {
		...
		return `${this._getRenderedSectionHeader({mon, title: "传奇动作", prop: "legendary"})}
	...
	_getCommonHtmlParts_mythicActions ({mon, entsMythicAction}) {
		...
		return `${this._getRenderedSectionHeader({mon, title: "神话动作", prop: "mythic"})}
	...
	_getCommonHtmlParts_lairActions ({legGroup}) {
		return `${legGroup && legGroup.lairActions ? `<tr><td colspan="6"><h3 class="ve-stats__sect-header-inner ve-stats__sect-header-inner--non-statblock">巢穴动作</h3></td></tr>
	...
	_getCommonHtmlParts_regionalEffects ({legGroup}) {
		return `${legGroup && legGroup.regionalEffects ? `<tr><td colspan="6"><h3 class="ve-stats__sect-header-inner ve-stats__sect-header-inner--non-statblock">区域效应</h3></td></tr>
	...
	_getHtmlPtEnvironment (mon) {
		...
		return `<div><b>${this._style === "classic" ? "环境" : "栖息地"}:</b> ${Renderer.monster.getRenderedEnvironment(mon.environment)}</div>`;
	}

	_getHtmlPtTreasure (mon) {
		if (!mon.treasure?.length) return "";
		return `<div><b>宝藏:</b> ${Renderer.monster.getRenderedTreasure(mon.treasure)}</div>`;
	}
...
class _RenderBestiaryImplClassic extends _RenderBestiaryImplBase {
	...
	_getHtmlParts_armorClass ({mon, renderer, isInlinedToken}) {
		return `<tr><td colspan="6"><div ${isInlinedToken ? `class="ve-stats__wrp-avoid-token"` : ""}><strong>护甲类型</strong> ${mon.ac == null ? "\u2014" : Parser.acToFull(mon.ac, {renderer})}</div></td></tr>`;
	}
	...
	_getRenderedCreature ({mon, opts, renderer}) {
		...
		<tr>${opts.selSummonSpellLevel ? veT`<td colspan="6"><strong class="ve-mr-2">法术环阶</strong> ${opts.selSummonSpellLevel}</td>` : ""}</tr>
		<tr>${opts.selSummonClassLevel ? veT`<td colspan="6"><strong class="ve-mr-2">${opts.classLevelScalerClass ? "职业等级" : "等级"}</strong> ${opts.selSummonClassLevel}</td>` : ""}</tr>
	...
	_getHtmlParts_armorClass ({mon, renderer, isInlinedToken}) {
		return `<tr><td colspan="6">
			<div class="ve-split-v-center ve-lst-is-exporting-image__no-wrap ${isInlinedToken ? `ve-stats__wrp-avoid-token` : ""}">
				<div><strong title="护甲等级">AC</strong> ${mon.ac == null ? "\u2014" : Parser.acToFull(mon.ac, {renderer})}</div>
...
export class RenderBestiary {
	...
	static getRenderedLegendaryGroup (legGroup) {
		...
		<tr><td colspan="6">
			...
			${legGroup.mythicEncounter && legGroup.mythicEncounter.length ? Renderer.get().render({type: "entries", entries: [{type: "entries", name: `<i title="当把传奇组应用在其他生物的数据卡中时，这里会显示生物的名字" class="ve-help-subtle">将&lt;生物名字&gt;</i>作为一次神话遭遇`, entries: legGroup.mythicEncounter}]}) : ""}
		</td></tr>
		${Renderer.utils.getBorderTr()}`;
	}
}
