...
class SaveManager extends BaseComponent {
	...
	async pDoLoad (
		...
		const btnExportAll = ee`<button class="ve-btn ve-btn-default ve-btn-xs" title="导出所有列表到文件">全部导出</button>`
			.onn("click", () => {
		...
		const btnImportAll = this._isReadOnlyUi
			? null
			: ee`<button class="ve-btn ve-btn-default ve-btn-xs" title="从文件导入所有列表">全部导入</button>`
				.onn("click", async () => {
					const {jsons, errors} = await InputUiUtil.pGetUserUploadJson({
		...
		const dispNoSaves = ee`<div class="ve-flex-col"><i class="ve-muted ve-text-center">未找到已保存的内容。</i></div>`;
	...
	getNewRender (save, i) {
		...
		const hkIsExpanded = () => {
			...
			btnExpand
				...
				.tooltip(comp._state.manager_loader_isExpanded ? "折叠预览" : "展开预览");
		...
		const btnLoad = ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-primary" title="加载"><span class="glyphicon glyphicon-ok"></span></button>`
			.onn("click", evt => {
				evt.stopPropagation();
		...
		const btnDelete = this._isReadOnlyUi
			? null
			: ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-danger" title="删除"><span class="glyphicon glyphicon-trash"></span></button>`
				.onn("click", evt => {
					evt.stopPropagation();
...
SaveManager._RenderableCollectionSaves_Summary = class extends RenderableCollectionBase {
	...
	getNewRender (save, i) {
		...
		const btnNew = ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-default" title="新建固定列表"><span class="glyphicon glyphicon-file"></span></button>`
			.onn("click", evt => this._cbOnNew(evt));

		const btnDuplicate = ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-default" title="拷贝固定列表"><span class="glyphicon glyphicon-duplicate"></span></button>`
			.onn("click", evt => this._cbOnDuplicate(evt));

		const btnSave = ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-default" title="保存固定列表"><span class="glyphicon glyphicon-floppy-disk"></span></button>`
			.onn("click", evt => this._cbOnSave(evt));

		const btnLoad = ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-default" title="加载固定列表"><span class="glyphicon glyphicon-folder-open"></span></button>`
		...
		const btnUpload = ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-default" title="上传固定列表"><span class="glyphicon glyphicon-upload"></span></button>`
			.onn("click", evt => this._cbOnUpload(evt));

		const btnReset = ee`<button class="ve-btn ve-btn-5et ve-btn-xs ve-btn-default" title="重置固定列表"><span class="glyphicon glyphicon-refresh"></span></button>`
