...
export class BuilderBase extends ProxyBase {
	...
	renderInputControls () {
		...
		const btnSave = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2 mkbru__cnt-save">保存</button>`
			.onn("click", () => this._pHandleClick_pSaveBrew());
		this._addHook("meta", "isModified", () => btnSave.txt(this._meta.isModified ? "保存 *" : "已保存"))();

		const btnNew = ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="SHIFT to reset additional state (such as whether or not certain attributes are auto-calculated)">新建</button>`
			.onn("click", async (evt) => {
	...
	getFluffInput (cb) {
		...
		const btnAddImage = ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增图片</button>`
			.onn("click", async () => {
