...
export class BuilderBase extends ProxyBase {
	...
	renderInputControls () {
		...
		const hkBtnSaveText = () => btnSave.txt(this._meta.isModified ? "保存 *" : "已保存");
		this._addHook("meta", "isModified", hkBtnSaveText);
		hkBtnSaveText();

		ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="SHIFT to reset additional state (such as whether or not certain attributes are auto-calculated)">新建</button>`
			.onn("click", async (evt) => {
