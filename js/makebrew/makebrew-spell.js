...
export class SpellBuilder extends BuilderBase {
	...
	_renderInputMain () {
		...
		BuilderUi.getStateIptEnum("环节", cb, this._state, {nullable: false, fnDisplay: (it) => Parser.spLevelToFull(it), vals: [...new Array(21)].map((_, i) => i)}, "level").vee.appendTo(infoTab.wrpTab);
		BuilderUi.getStateIptEnum("学派", cb, this._state, {nullable: false, fnDisplay: (it) => Parser.spSchoolAbvToFull(it), vals: [...Parser.SKL_ABVS]}, "school").vee.appendTo(infoTab.wrpTab);
		...
		BuilderUi.getStateIptEntries("说明", cb, this._state, {fnPostProcess: BuilderUi.fnPostProcessDice}, "entries").vee.appendTo(detailsTab.wrpTab);
		...
		veT`<div class="ve-flex-vh-center ve-w-100 ve-mb-2"><i>注意：以下数据会用于法术页面的筛选功能。</i></div>`.vee.appendTo(miscTab.wrpTab);
	...
	__getOtherSourcesInput (cb) {
		...
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">添加其他来源</button>`
	...
	__getOtherSourcesInput__getOtherSourceRow (doUpdateState, otherSourceRows, os) {
		...
		const wrp = veT`<div class="ve-flex-col mkbru__wrp-rows mkbru__wrp-rows--removable">
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">来源</span>${compSelSource.getWrp()}</div>
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">页码</span>${iptPage}</div>
			${wrpBtnRemove}
		</div>`;
	...
	__getTimeInput (cb) {
		...
		const wrpBtnAdd = veT`<div></div>`.vee.appendTo(rowInner);
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">添加施法时间</button>`
			.vee.appendTo(wrpBtnAdd)
			.vee.onn("click", () => {
	...
	static __getTimeInput__getTimeRow (doUpdateState, timeRows, time) {
		...
		const iptCond = veT`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="当你做...时触发">`
			.vee.onn("change", () => doUpdateState())
	...
	__getRangeInput (cb) {
		...
		veT`<div class="ve-flex-v-center">
			<span class="ve-mr-2 mkbru__sub-name--33">范围类型</span>
			${selRange}
		...
		const stageDistance = veT`<div class="ve-flex-v-center ve-mt-2">
			<span class="ve-mr-2 mkbru__sub-name--33">距离类型</span>
			${selDistance}
		...
		const stageAmount = veT`<div class="ve-flex-v-center ve-mt-2">
			<span class="ve-mr-2 mkbru__sub-name--33">距离数值</span>
			${iptAmount}
	...
	__getComponentInput (cb) {
		...
		veT`<div>
			<div class="ve-flex-v-center ve-mb-2"><div class="ve-mr-2 mkbru__sub-name--33">言语</div>${cbVerbal}</div>
			<div class="ve-flex-v-center ve-mb-2"><div class="ve-mr-2 mkbru__sub-name--33">姿势</div>${cbSomatic}</div>
			<div class="ve-flex-v-center ve-mt-2"><div class="ve-mr-2 mkbru__sub-name--33">版税</div>${cbRoyalty}</div>
			<div class="ve-flex-v-center"><div class="ve-mr-2 mkbru__sub-name--33">材料类型</div>${selMaterial}</div>
		</div>`.vee.appendTo(rowInner);

		// BASIC MATERIAL
		const stageMaterial = veT`<div class="ve-flex-v-center ve-mt-2"><div class="ve-mr-2 mkbru__sub-name--33">材料</div>${iptMaterial}</div>`.vee.appendTo(rowInner).vee.toggle(initialMaterialMode === "1" || initialMaterialMode === "2");
		...
		const stageMaterialConsumable = veT`<div class="ve-mt-2">
			<div class="ve-flex-v-center ve-mb-2"><div class="ve-mr-2 mkbru__sub-name--33 ve-help" title="${TITLE_FILTERS_EXTERNAL}">是消耗品？</div>${cbConsumed}</div>
			<div class="ve-flex-v-center"><div class="ve-mr-2 mkbru__sub-name--33 ve-help" title="${TITLE_FILTERS_EXTERNAL}以铜币来计算(1gp = 100cp).">成分价值</div>${iptCost}<div>cp</div></div>
	...
	__getMetaInput (cb) {
		...
		veT`<div>
			<div class="ve-flex-v-center ve-mb-2"><div class="ve-mr-2 mkbru__sub-name--33">仪式</div>${cbRitual}</div>
			<div class="ve-flex-v-center"><div class="ve-mr-2 mkbru__sub-name--33">技术魔法</div>${cbTechnomagic}</div>
		</div>`.vee.appendTo(rowInner);
	...
	__getDurationInput (cb) {
		...
		const wrpBtnAdd = veT`<div></div>`.vee.appendTo(rowInner);
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">添加持续时间</button>`
			.vee.appendTo(wrpBtnAdd)
			.vee.onn("click", () => {
	...
	static __getDurationInput__getDurationRow (doUpdateState, durationRows, duration) {
		...
		const stageAmount = veT`<div class="ve-flex-col ve-mb-2">
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">专注</span>${cbConc}</div>
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33 ve-help" title="对于需要专注的法术，此选项无效。因为那些法术可以通过结束专注状态来随时终止。">至多...</span>${cbUpTo}</div>
		...
		const wrpBtnRemove = veT`<div class="ve-text-right ve-mb-2"></div>`;
		const wrp = veT`<div class="ve-flex-col mkbru__wrp-rows mkbru__wrp-rows--removable">
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">持续时间类型</span>${selDurationType}</div>
			${stageAmount}
			${stageEnds}
			${wrpBtnRemove}
		</div>`;
	...
	__getClassesInputs (cb) {
		...
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">添加职业</button>`
		...
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">添加子职</button>`
	...
	__getClassesInputs__getClassRow (doUpdateState, classRows, cls) {
		...
		const wrpBtnRemove = veT`<div class="ve-text-right ve-mb-2"></div>`;
		const wrp = veT`<div class="ve-flex-col mkbru__wrp-rows mkbru__wrp-rows--removable">
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">职业名字</span>${iptClass}</div>
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">职业来源</span>${compSelSource.getWrp()}</div>
			${wrpBtnRemove}
		</div>`;
	...
	__getRaces (cb) {
		...
		const wrpBtnAdd = veT`<div></div>`.vee.appendTo(rowInner);
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">添加种族</button>`
			.vee.appendTo(wrpBtnAdd)
			.vee.onn("click", () => {
	...
	__getRaces__getRaceRow (doUpdateState, raceRows, race) {
		...
		const wrpBtnRemove = veT`<div class="ve-text-right ve-mb-2"></div>`;
		const wrp = veT`<div class="ve-flex-col mkbru__wrp-rows">
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">名称</span>${iptRace}</div>
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">来源</span>${compSelSource.getWrp()}</div>
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33 ve-help" title="基础种族的名字，例如“精灵”。这个主要用于筛选功能。">基础种族</span>${iptBaseRace}</div>
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33 ve-help" title="例如“精灵”的基础来源是“${Parser.SRC_PHB}”。">基础来源</span>${compSelSourceBase.getWrp()}</div>
			${wrpBtnRemove}
		</div>`;
	...
	__getSimpleSource ({cb, nameSingle, namePlural, prop, propTracker}) {
		...
		const wrpBtnAdd = veT`<div></div>`.vee.appendTo(rowInner);
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">添加${nameSingle}</button>`
			.vee.appendTo(wrpBtnAdd)
			.vee.onn("click", () => {
	...
	__getSimpleSource__getIdentRow (doUpdateState, rows, identObj, {nameSingle, propTracker}) {
		...
		const wrpBtnRemove = veT`<div class="ve-text-right ve-mb-2"></div>`;
		const wrp = veT`<div class="ve-flex-col mkbru__wrp-rows">
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">名称</span>${iptName}</div>
			<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">来源</span>${compSelSource.getWrp()}</div>
			${wrpBtnRemove}
		</div>`;
	...
	__getSourcesGenerated (cb, fnsDoRefreshSources) {
		...
		const getBtnAdd = () => {
			const btn = veT`<button class="ve-btn ve-btn-xs ve-btn-default" title="基于当前法术的来源生成额外来源（例如，为法师法术生成奥法骑士战士来源）。">添加额外来源</button>`
				.vee.onn("click", async () => {
					try {
