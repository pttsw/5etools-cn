...
export class SpellBuilder extends BuilderBase {
	...
	_renderInputMain () {
		...
		ee`<div class="ve-flex-vh-center ve-w-100 ve-mb-2"><i>注意：以下数据会用于法术页面的筛选功能。</i></div>`.appendTo(miscTab.wrpTab);
	...
	static __getTimeInput__getTimeRow (doUpdateState, timeRows, time) {
		...
		const iptCond = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="当你做...时触发">`
			.onn("change", () => doUpdateState())
			.val(time.condition);
	...
	__getComponentInput (cb) {
		...
		// BASIC MATERIAL
		const stageMaterial = ee`<div class="ve-flex-v-center ve-mt-2"><div class="ve-mr-2 mkbru__sub-name--33">材料</div>${iptMaterial}</div>`.appendTo(rowInner).toggleVe(initialMaterialMode === "1" || initialMaterialMode === "2");
