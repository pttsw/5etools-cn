...
export class BuilderUi {
	...
	static getStateIptEnum (name, fnRender, state, options, ...path) {
		...
		if (options.nullable) sel.vee.appends(`<option value="-1">(无)</option>`);
	...
	static getSplitCommasSortButton (ipt, cb, sortOptions = null) {
		...
		return veT`<button class="ve-btn ve-btn-xs ve-btn-default">排序</button>`
			.vee.onn("click", () => {
	...
	static getDragPad (cbUpdate, rows, myRow, options) {
		...
		return veT`<div class="ve-ml-2 ve-ui-drag__patch" title="拖拽排序">
