...
export class CreatureBuilder extends BuilderBase {
	...
	doHandleSourcesAdd () {
		(this._eles.selVariantSources || []).map(sel => {
			...
			sel.empty().appends(`<option value="">(与此生物相同)</option>`);
	...
	__getSizeInput (cb) {
		...
		const btnAddSize = ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增体型</button>`
			.onn("click", () => {
	...
	__getTypeInput (cb) {
		...
		const btnAddChooseType = ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增类型</button>`
			.onn("click", () => {
		...
		const btnAddTag = ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增标签</button>`
			.onn("click", () => {
		...
		ee`<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">标签内容</span>${iptNote}</div>`
			.appendTo(rowInner);
		// endregion
	...
	__getTypeInput__getChooseTypeRow (type, chooseTypeRows, setState) {
		...
		const iptTypeCustom = ee`<input class="ve-form-control ve-input-xs form-control--minimal ve-mr-2" placeholder="自定义类型">`
			.onn("change", () => {
				setState();
			});
	...
	__getAlignmentInput (cb) {
		...
		ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增阵营</button>`
			.appendTo(wrpBtnAdd)
			.onn("click", () => {
	...
	static __getAlignmentInput__getAlignmentRow (doUpdateState, alignmentRows, alignment) {
		...
		// MULTIPLE CONTROLS
		const iptChance = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" min="1" max="100" placeholder="阵营可能性">`
			.onn("change", () => doUpdateState());
		const iptNote = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mx-1" placeholder="阵营标注">`
			.onn("change", () => doUpdateState());
	...
	__getAcInput (cb) {
		...
		const wrpBtnAdd = ee`<div></div>`.appendTo(rowInner);
		ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增护甲等级来源</button>`
			.appendTo(wrpBtnAdd)
			.onn("click", () => {
	...
	static __getAcInput__getAcRow (ac, acRows, doUpdateState) {
		...
		const iptCond = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="当...时">`
			.onn("change", () => doUpdateState());
		...
		const btnAddFrom = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mb-2">新增另一个特性/物品</button>`
			.onn("click", () => {
	...
	static __getAcInput__getFromRow (from, fromRows, doUpdateState) {
		...
		const iptFrom = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" placeholder="来自于...">`
			.onn("change", () => doUpdateState());
		...
		const btnCommon = ee`<button class="ve-btn ve-btn-default ve-btn-xs ve-mr-2">特性 <span class="caret"></span></button>`
			.onn("click", evt => ContextUtil.pOpenMenu(evt, menu));

		const btnSearchItem = ee`<button class="ve-btn ve-btn-default ve-btn-xs">物品</button>`
			.onn("click", () => {
	...
	__getHpInput (cb) {
		...
		const btnAutoSimpleFormula = ee`<button class="ve-btn ve-btn-xs ve-btn-default ${this._meta.autoCalc.hpModifier ? "ve-active" : ""}" title="根据体质值自动计算"><span class="glyphicon glyphicon-refresh"></span></button>`
			.onn("click", () => {
				if (this._meta.autoCalc.hpModifier) {
					this._meta.autoCalc.hpModifier = false;
					this.doUiSave();
	...
	__getSpeedInput (cb) {
		...
		const getRow = (name, prop) => {
			...
			const iptCond = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="${prop === "fly" ? "(漂浮)/当...时" : "当...时"}">`
				.onn("change", () => doUpdateProp());
	...
	__getDefensesInput (cb, rowName, shortName, prop) {
		...
		const btnAddGroup = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2">新增此类别</button>`
	...
	static __getDefensesInput__getNodeGroup (shortName, prop, groups, doUpdateState, depth, initial) {
		...
		const btnAddChild = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2">新增${shortName}</button>`
			.onn("click", (evt) => ContextUtil.pOpenMenu(evt, menu));
		const btnAddChildGroup = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2">新增子类别</button>`
			.onn("click", () => addChild(CreatureBuilder.__getDefensesInput__getNodeGroup(shortName, prop, children, doUpdateState, depth + 1)));
		const iptNotePre = ee`<input class="ve-form-control ve-input-xs form-control--minimal ve-mr-2" placeholder="前缀">`
			.onn("change", () => doUpdateState());
		const iptNotePost = ee`<input class="ve-form-control ve-input-xs form-control--minimal ve-mr-2" placeholder="后缀">`
			.onn("change", () => doUpdateState());
	...
	__getSenseInput (cb) {
		...
		const btnAddGeneric = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2 mkbru_mon__btn-add-sense-language">新增感知</button>`
			.onn("click", (evt) => ContextUtil.pOpenMenu(evt, menu));
	...
	__getLanguageInput (cb) {
		...
		const btnAddGeneric = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2 mkbru_mon__btn-add-sense-language">新增语言</button>`
			.onn("click", async () => {
	...
	__getCrInput (cb) {
		...
		const stageLair = ee`<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">在巢穴中时</span>${selCrLair}</div>`
		...
		const stageCoven = ee`<div class="ve-flex-v-center ve-mb-2"><span class="ve-mr-2 mkbru__sub-name--33">属于集会时</span>${selCrCoven}</div>`
	...
	__getSpellcastingInput (cb) {
		...
		const btnAddRow = ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增施法特质</button>`
			.appendTo(wrpControls)
			.onn("click", () => {
				doAddTrait();
				doUpdateState();
	...
	__getSpellcastingInput__getTraitRow (traitRows, doUpdateState, trait) {
		...
		const iptName = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" placeholder="特质名称">`
			.onn("change", () => doUpdateState());
		iptName.val(trait ? trait.name : "施法");

		const btnToggleHeader = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2">标题</button>`
			.onn("click", () => {
		...
		const btnToggleFooter = ee`<button class="ve-btn ve-btn-xs ve-btn-default ve-mr-2">后缀</button>`
			.onn("click", () => {
		...
		const btnAddSpell = ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增...</button>`
			.onn("click", (evt) => ContextUtil.pOpenMenu(evt, menu));

		const iptHeader = ee`<textarea class="ve-form-control form-control--minimal ve-resize-vertical ve-mb-2" placeholder="标题文本"></textarea>`
			.toggleVe(!!(trait && trait.headerEntries))
			.onn("change", () => doUpdateState());
		if (trait && trait.headerEntries) iptHeader.val(UiUtil.getEntriesAsText(trait.headerEntries));

		const iptFooter = ee`<textarea class="ve-form-control form-control--minimal ve-resize-vertical ve-mb-2" placeholder="后缀文本"></textarea>`
	...
	__getSpellcastingInput__getSpellGenericRow (spellRows, doUpdateState, meta, data) {
		...
		const btnAdd = ee`<button class="ve-btn ve-btn-xxs ve-btn-default ve-mr-2" title="新增法术"><span class="glyphicon glyphicon-plus"></span></button>`
			.onn("click", async () => {
		...
		const metaPart = (() => {
			...
			switch (meta.mode) {
				case "basic": {
					out.ele = ee`<i>${meta.type === "constant" ? "永久效应" : "随意"}</i>`;
				...
				case "level": {
					...
					out.ele = ee`<div class="ve-flex mkbru_mon__spell-header-wrp ve-mr-4">
					<div class="ve-italic">${Parser.spLevelToFull(meta.level)}法术</div>
					...
					<label class="ve-flex-v-center ve-muted small"><span class="ve-mr-1">魔契师?</span>${cbWarlock}<span>)</span></label>
					</div>`;
				...
				generators: [
					{
						...
						action: () => {
							return new Promise(resolve => {
								...
								const iptName = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" placeholder="武器">`;
								...
								const iptMeleeDamBonus = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" placeholder="+X (额外伤害)">`;
								const iptMeleeDamType = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="近战伤害类型" autocomplete="off">`
								...
								const iptRangedDamBonus = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" placeholder="+X (额外伤害)">`;
								const iptRangedDamType = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="远程伤害类型">`
									.typeahead(Parser.DMG_TYPES);
								...
								const iptVersatileDamBonus = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" placeholder="+X (额外伤害)">`;
								const iptVersatileDamType = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="两用伤害类型">`
									.typeahead(Parser.DMG_TYPES);
								...
								const iptBonusDamBonus = ee`<input class="ve-form-control form-control--minimal ve-input-xs ve-mr-2" placeholder="+X (额外伤害)">`;
								const iptBonusDamType = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="额外伤害类型">`
									.typeahead(Parser.DMG_TYPES);
								...
								const btnConfirm = ee`<button class="ve-btn ve-btn-sm ve-btn-default ve-mr-2">新增</button>`
									.onn("click", () => {
								...
								const btnReset = ee`<button class="ve-btn ve-btn-sm ve-btn-danger">重置</button>`
									.onn("click", async () => {
	...
	__getGenericEntryInput (cb, options) {
		...
		const wrpBtnAdd = ee`<div></div>`.appendTo(rowInner);
		ee`<button class="ve-btn ve-btn-xs ve-btn-default">新增${options.shortName}</button>`
			.appendTo(wrpBtnAdd)
			.onn("click", () => {
	...
	__getGenericEntryInput__getEntryRow (doUpdateState, doUpdateOrder, options, entryRows, entry) {
		...
		const iptName = ee`<input class="ve-form-control form-control--minimal ve-input-xs" placeholder="${options.shortName}名称">`
			.onn("change", () => doUpdateState());
	...
	__getLegendaryGroupInput (cb) {
		...
		this._selLegendaryGroup = ee`<select class="ve-form-control form-control--minimal ve-input-xs"><option value="-1">无</option></select>`
			.onn("change", () => {
	...
	__getEnvironmentInput (cb) {
		...
		const btnAddCustom = ee`<button class="ve-btn ve-btn-default ve-btn-xs ve-mt-2">新增自定义环境</button>`
			.onn("click", () => {
