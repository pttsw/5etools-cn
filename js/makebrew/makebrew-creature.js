...
export class CreatureBuilder extends BuilderBase {
	...
	__getTypeInput (cb) {
		...
		ee`<div class="ve-flex-v-center mb-2"><span class="mr-2 mkbru__sub-name--33">标签内容</span>${iptNote}</div>`
			.appendTo(rowInner);
		// endregion
	...
	__getTypeInput__getChooseTypeRow (type, chooseTypeRows, setState) {
		...
		const iptTypeCustom = ee`<input class="form-control input-xs form-control--minimal mr-2" placeholder="自定义类型">`
			.onn("change", () => {
				setState();
			});
	...
	static __getAlignmentInput__getAlignmentRow (doUpdateState, alignmentRows, alignment) {
		...
		// MULTIPLE CONTROLS
		const iptChance = ee`<input class="form-control form-control--minimal input-xs mr-2" min="1" max="100" placeholder="阵营可能性">`
			.onn("change", () => doUpdateState());
		const iptNote = ee`<input class="form-control form-control--minimal input-xs mx-1" placeholder="阵营标注">`
			.onn("change", () => doUpdateState());
	...
	static __getAcInput__getAcRow (ac, acRows, doUpdateState) {
		...
		const iptCond = ee`<input class="form-control form-control--minimal input-xs" placeholder="当...时">`
			.onn("change", () => doUpdateState());
		...
		const btnAddFrom = ee`<button class="ve-btn ve-btn-xs ve-btn-default mb-2">新增另一个特性/物品</button>`
			.onn("click", () => {
	...
	static __getAcInput__getFromRow (from, fromRows, doUpdateState) {
		...
		const iptFrom = ee`<input class="form-control form-control--minimal input-xs mr-2" placeholder="来自于...">`
			.onn("change", () => doUpdateState());
		...
		const btnCommon = ee`<button class="ve-btn ve-btn-default ve-btn-xs mr-2">特性 <span class="caret"></span></button>`
	...
	__getHpInput (cb) {
		...
		const btnAutoSimpleFormula = ee`<button class="ve-btn ve-btn-xs ve-btn-default ${this._meta.autoCalc.hpModifier ? "active" : ""}" title="根据体质值自动计算"><span class="glyphicon glyphicon-refresh"></span></button>`
			.onn("click", () => {
				if (this._meta.autoCalc.hpModifier) {
					this._meta.autoCalc.hpModifier = false;
					this.doUiSave();
	...
	__getSpeedInput (cb) {
		...
		const getRow = (name, prop) => {
			...
			const iptCond = ee`<input class="form-control form-control--minimal input-xs" placeholder="${prop === "fly" ? "(漂浮)/当...时" : "当...时"}">`
				.onn("change", () => doUpdateProp());
	...
	__getDefensesInput (cb, rowName, shortName, prop) {
		...
		const btnAddGroup = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2">新增此类别</button>`
	...
	static __getDefensesInput__getNodeGroup (shortName, prop, groups, doUpdateState, depth, initial) {
		...
		const btnAddChild = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2">新增${shortName}</button>`
			.onn("click", (evt) => ContextUtil.pOpenMenu(evt, menu));
		const btnAddChildGroup = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2">新增子类别</button>`
			.onn("click", () => addChild(CreatureBuilder.__getDefensesInput__getNodeGroup(shortName, prop, children, doUpdateState, depth + 1)));
		const iptNotePre = ee`<input class="form-control input-xs form-control--minimal mr-2" placeholder="前缀">`
			.onn("change", () => doUpdateState());
		const iptNotePost = ee`<input class="form-control input-xs form-control--minimal mr-2" placeholder="后缀">`
			.onn("change", () => doUpdateState());
	...
	__getSenseInput (cb) {
		...
		const btnAddGeneric = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2 mkbru_mon__btn-add-sense-language">新增感知</button>`
			.onn("click", (evt) => ContextUtil.pOpenMenu(evt, menu));
	...
	__getLanguageInput (cb) {
		...
		const btnAddGeneric = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2 mkbru_mon__btn-add-sense-language">新增语言</button>`
			.onn("click", async () => {
	...
	__getCrInput (cb) {
		...
		const stageLair = ee`<div class="ve-flex-v-center mb-2"><span class="mr-2 mkbru__sub-name--33">在巢穴中时</span>${selCrLair}</div>`
		...
		const stageCoven = ee`<div class="ve-flex-v-center mb-2"><span class="mr-2 mkbru__sub-name--33">属于集会时</span>${selCrCoven}</div>`
	...
	__getSpellcastingInput__getTraitRow (traitRows, doUpdateState, trait) {
		...
		const iptName = ee`<input class="form-control form-control--minimal input-xs mr-2" placeholder="特质名称">`
			.onn("change", () => doUpdateState());
		iptName.val(trait ? trait.name : "施法");

		const btnToggleHeader = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2">标题</button>`
			.onn("click", () => {
		...
		const btnToggleFooter = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2">后缀</button>`
			.onn("click", () => {
		...
		const iptHeader = ee`<textarea class="form-control form-control--minimal resize-vertical mb-2" placeholder="标题文本"></textarea>`
			.toggleVe(!!(trait && trait.headerEntries))
			.onn("change", () => doUpdateState());
		if (trait && trait.headerEntries) iptHeader.val(UiUtil.getEntriesAsText(trait.headerEntries));

		const iptFooter = ee`<textarea class="form-control form-control--minimal resize-vertical mb-2" placeholder="后缀文本"></textarea>`
	...
	__getSpellcastingInput__getSpellGenericRow (spellRows, doUpdateState, meta, data) {
		...
		const btnAdd = ee`<button class="ve-btn ve-btn-xxs ve-btn-default mr-2" title="新增法术"><span class="glyphicon glyphicon-plus"></span></button>`
			.onn("click", async () => {
				...
				generators: [
					{
						...
						action: () => {
							return new Promise(resolve => {
								...
								const iptName = ee`<input class="form-control form-control--minimal input-xs mr-2" placeholder="武器">`;
								...
								const btnConfirm = ee`<button class="ve-btn ve-btn-sm ve-btn-default mr-2">新增</button>`
									.onn("click", () => {
	...
	__getGenericEntryInput__getEntryRow (doUpdateState, doUpdateOrder, options, entryRows, entry) {
		...
		const iptName = ee`<input class="form-control form-control--minimal input-xs" placeholder="${options.shortName}名称">`
			.onn("change", () => doUpdateState());
	...
	__getLegendaryGroupInput (cb) {
		...
		this._selLegendaryGroup = ee`<select class="form-control form-control--minimal input-xs"><option value="-1">无</option></select>`
			.onn("change", () => {
	...
	__getEnvironmentInput (cb) {
		...
		const btnAddCustom = ee`<button class="ve-btn ve-btn-default ve-btn-xs mt-2">新增自定义环境</button>`
			.onn("click", () => {
