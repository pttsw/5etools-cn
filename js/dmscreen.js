...
class SideMenu {
	...
	render () {
		...
		const $btnSetDim = $(`<button class="ve-btn ve-btn-primary" style="width: 100%;">设置尺寸</div>`).appendTo($wrpSetDim);
		...
		const $btnFullscreen = $(`<button class="ve-btn ve-btn-primary">切换全屏</button>`).appendTo($wrpFullscreen);
		...
		const $btnSaveFile = $(`<button class="ve-btn ve-btn-primary">保存到文件</button>`).appendTo($wrpSaveLoadFile);
		$btnSaveFile.on("click", () => {
			DataUtil.userDownload(`dm-screen`, this.board.getSaveableState(), {fileType: "dm-screen"});
		});
		const $btnLoadFile = $(`<button class="ve-btn ve-btn-primary">从文件加载</button>`).appendTo($wrpSaveLoadFile);
		...
		const $btnSaveLink = $(`<button class="ve-btn ve-btn-primary">保存为URL</button>`).appendTo($wrpSaveLoadUrl);
...
class AddMenuSearchTab extends AddMenuTab {
	...
	async pRender () {
		...
		if (!this.$tab) {
			...
			const iptSearch = ee`<input class="ui-search__ipt-search search form-control" autocomplete="off" placeholder="搜索...">`.appendTo($wrpCtrls[0]);
