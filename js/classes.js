...
class ClassesPage extends MixinComponentGlobalState(MixinBaseComponent(MixinProxyBase(ListPage))) {
	...
	_render_renderClassTable_renderTableGroupHeader (
		...
		if (tableGroup.rowsSpellProgression) {
			...
			tblHeaderSpellPointsMaxSpellLevel = ee`<th class="cls-tbl__col-generic-center cls-tbl__cell-spell-points"><div class="cls__squash_header">法术环阶</div></th>`;
	...
	_render_renderSubclassPrimaryControls (wrp) {
		...
		this._listSubclass.on("updated", () => {
			...
			if (this._listSubclass.visibleItems.length) {
				...
				dispCount
					.html(cntNotShown ? `<i class="clickable" title="修改筛选条件来查看">(${cntNotShown}个未显示)</i>` : "")
			...
			if (this._listSubclass.items.length > 1) {
				dispCount
					.html(`<i class="clickable" title="修改筛选条件来查看。">(${this._listSubclass.items.length - 1}个子职未显示)</i>`)
					.onn("click", () => this._doSelectAllSubclasses());
				return;
			}
	...
	async _render_pInitSubclassControls (wrp) {
		...
		const selFilterPreset = ee`<select class="input-xs form-control cls-tabs__sel-preset"><option value="-1" disabled>筛选...</option></select>`
			.onn("change", () => {
	...
	async _render_pRenderClassContent () {
		...
		if (cls.otherSources) {
			...
			const trClassFeature = ee`<tr data-feature-type="class"></tr>`
				.html(`<td colspan="6"><hr class="hr-1"><b>职业来源：</b> ${htmlSource}</td>`)
				.appendTo(wrpContent);
		}
	...
	async _render_renderClassContent_pRenderFeature (
		...
		const trSubclassFeature = ee`<tr class="cls-main__sc-feature" data-subclass-none-message="true"></tr>`
			.html(`<td colspan="6">${Renderer.get().withDepthTracker([], ({renderer}) => renderer.render({type: "entries", entries: [{name: `{@note 未选择子职业}`, type: "entries", entries: [`{@note <span class="clickable roller" data-jump-select-a-subclass="true">选择一个子职业</span> 来在此处查看特性。}`]}]}))}</td>`)
			.appendTo(wrpContent);
...
ClassesPage.SubclassComparisonBookView = class extends BookModeViewBase {
	...
	_getSelectSubclassesMeta ({cbOnSave = null, isCloseButton = true} = {}) {
		...
		const stg = ee`<div class="ve-flex-col">
			<div class="mb-2 initial-message initial-message--med">请选择一些子职：</div>
			${wrpRows}
			${dispNoneAvailable}
		</div>`;
		...
		const onListUpdate = () => {
			...
			const btnSave = ee`<button class="ve-btn ve-btn-default mr-2" data-i18n="common.button.save">Save</button>`
				.onn("click", async () => {
