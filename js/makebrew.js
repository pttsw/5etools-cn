...
class PageUi {
	...
	async _pInitSideMenu () {
		...
		const wrpMode = ee`<div class="ve-w-100 ve-split-v-center"><div class="sidemenu__row__label ve-mr-2">模式</div></div>`.appendTo(mnu);
		this._selBuilderMode = ee`
			<select class="ve-form-control ve-input-xs">
				...
				<option value="none" class="ve-italic">其他内容？</option>
			</select>
		`
			.appendTo(wrpMode)
			.onn("change", async () => {
		...
		const wrpSource = ee`<div class="ve-w-100 ve-mb-2 ve-split-v-center"><div class="sidemenu__row__label ve-mr-2">来源</div></div>`.appendTo(mnu);
		...
		const btnSourceEdit = ee`<button class="ve-btn ve-btn-default ve-btn-xs ve-mr-2">编辑当前来源</button>`
			.onn("click", () => {
		...
		const btnSourceAdd = ee`<button class="ve-btn ve-btn-default ve-btn-xs">添加新来源</button>`.onn("click", () => {
