...
class PageUi {
	...
	async _pInitSideMenu () {
		...
		const wrpMode = ee`<div class="w-100 split-v-center"><div class="sidemenu__row__label mr-2">模式</div></div>`.appendTo(mnu);
		...
		const wrpSource = ee`<div class="w-100 mb-2 split-v-center"><div class="sidemenu__row__label mr-2">来源</div></div>`.appendTo(mnu);
		...
		const btnSourceEdit = ee`<button class="ve-btn ve-btn-default ve-btn-xs mr-2">编辑当前来源</button>`
			.onn("click", () => {
